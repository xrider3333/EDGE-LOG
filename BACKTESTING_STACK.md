...
- **2026-09-09 — MAIN WAS RED and the cause was an empty cache file, not a code change (`tests/test_cloud_signal.py`).** `HAS_REAL_CACHE` tested `os.path.exists` only, and **`C:\EdgeLog\ohlc\QQQ_1m.csv` is ZERO BYTES** (the 5m cache beside it is a healthy 444 KB), so the skipif passed and both replay tests died inside `pd.read_csv` with `EmptyDataError: No columns to parse from file`. Every push in the repo was blocked by the engine gate behind an error message that named pandas rather than the cache. The guard now requires a non-empty file and the skip reason names the offending path, which is the same convention `tests/test_etf_book_shadow.py` already uses for its missing master. **THE UNDERLYING QUESTION IS NOT FIXED HERE AND BELONGS TO THE FEED OWNER: why is the QQQ 1-minute cache empty?** It went to zero around the switch of that feed from yfinance to Webull; the cloud signal engine replays off this cache, so an empty 1m file means that path has no bars to replay.
