...
> the NOISE crown (`NOISE_1_1_NBHD.py` at run #304's own champion settings, NQ 5m RTH, 0.533/RT, x20,
> weight 1). Scored by the app's own book runner on 2010-06-07..2026-06-30 with the last 12 months held
> back: selection $1,395,904 on a $36,562 drawdown = **38.18 net/DD** (the superseded #361: 30.25),
> whole window **46.11** (36.93), held-back year 10.33 (10.31), **8 of 8 chronological slices**.
> Profit factor is LOWER than #361 (1.486 vs 1.526) and trade count roughly doubles - this book is
> judged on return per unit of drawdown, not on profit factor.
> **Reproduce/re-queue:** `python tools/queue_book_four_leg.py`. **Supersedes run #361.**
> **Standing caveat:** in every version of this book, #361 included, the held-back year's ten best
> days are about 103% of that year's profit, so the recent year is tail-driven whichever book is chosen.
...
- `tools/trade_anatomy.py` -> `docs/anatomy/<leg>_trades.csv|.md|_features.json|/*.png` (2026-09-07) — **TRADE ANATOMY, the exploratory half** (owner: 'assess the structure and dynamics of each trade before entry'). ~75 causal readings at the decision bar (last-12-bar shape, distance to prior-day H/L/C, opening range, VWAP, 5m/60m/daily MAs, prior-week range, RSI/ADX/ATR on 3 timeframes, gap, streaks, time of day, volume, prior N-bar move WITH the trade's direction), winner-vs-loser anatomy, depth-3 tree rules, skip-rule search, chart PNGs of the 16 best/worst trades, median price-path overlay. HARD RULE: discovery = first 60% of pre-lockbox by date; holdout = the rest, ONE pre-registered peek; lockbox never loaded. First run (NOISE #243 paper leg, ORB #314): every skip rule that helped in discovery 2010-19 LOST total money in holdout 2019-25 (per-trade averages rose, totals fell) = regime artifacts; ORB's path into a trade is identical for winners and losers (context-free confirmed). The ONE structural read that carries both windows: NOISE trades entered after a strong 2-hour move WITH the trade's direction (top third, >= ~4.3 ATR) earn $51 vs -$10 per trade in discovery and $245 vs $90 in holdout (9/10 + 5/7 years); as a 2x SIZE TILT MAR 0.97->1.37 and 2.50->2.86, PF up in both. Filters lose total money everywhere. Candidate = pre-registered NOISE momentum size tilt (owner call).
- `tools/feature_board.py` + `tools/exit_autopsy.py` → `docs/feature_board.json` / `docs/exit_autopsy.json`, read by **COMPARE ▸ FEATURE BOARD** (v73.538 board, v73.541 adds LIFT, 2026-09-07) — **CROSS-FAMILY FEATURE BOARD**: the same 33 causal features (bar-level, prior-day, macro, structure, 60m compression) scored against per-trade PnL on six crowned legs (ORB #314/#234, NOISE #243/#316, ENGU-Q #309, NQDIP #307; every leg reproduces its run doc to the dollar); a feature is PROMOTED only when it survives (q<0.10, block-bootstrap CI excludes 0, beats shadow probes) with the same sign in ≥3 families AND the lockbox sign repeats in each. First run: **0 PROMOTED, 6 WATCH** (all ENGU-Q + NQDIP/NOISE pairs; NQDIP survivors are its own dip mechanism; `curve` is a slow-macro trend artifact, lockbox disagrees). 60m compression survives only on NOISE (both legs) under a rank test — the round-6 ORB effect is tail-driven and a Spearman cannot see it. **LIFT** (v73.589) = mean R in the top third of a feature minus the bottom third, day-block bootstrap CI + day-level permutation p + BH + probes, the tail-aware second read the rank test lacks: compression lift is POSITIVE on 5/6 legs and in every lockbox (ORB +1.5R, ENGU-Q +2.6R on ~20 trades) but clears the bar only on NOISE #243 (+0.46R); `atr_norm` and `curve` are rank-vs-lift CONFLICTS (rank hurts, dollars help = the big winners live in high vol), capped at WATCH. Still 0 PROMOTED. EXIT AUTOPSY (MFE/MAE per trade): exits are NOT the lever on any leg (winners keep 45-85% of best move, only 8-24% of losers were ever up 1R). Caches gitignored; `tools/featboard_render_probe.py [--real]` is the render gate.
...
## Changelog
- **2026-09-26 - CORRECTION to the 2026-09-24 TTIBS closure: the book it quoted was wrong, the verdict stands.** BOOK #419 (and the NOISE swap cards #417/#418) ran their first four legs on DEFAULT_PARAMS, not BOOK #396/#366's frozen settings (ENGU-Q 2,843 trades / $420,506 vs 1,949 / $603,381). Re-run with frozen legs, every leg reproducing #396/#366 to the dollar: **BOOK #431 = #396 + TTIBS #300**: pre-lockbox $1,858,429 vs $1,483,223 (+25%), max DD $68,852 vs $36,562 (+88%), net/DD 26.99 vs 40.57, PF 1.526 vs 1.511; lockbox $343,905 vs $307,096, lockbox DD $48,074 vs $27,506 (+75%). Fails MAR x1.05 and the lockbox-DD clause - **TTIBS stays closed**. Roll audit (ROLL_AUDIT.md) moves TTIBS #300's cold lockbox $25,359 -> $16,499 and whole $393,623 -> $385,776, which only weakens it. **NOISE #382 leg swap**: BOOK #430 (#396 base) pre-lockbox $1,675,397 (+13.0%), DD $37,738 (+3.2%), net/DD 44.40 vs 40.57, lockbox $328,445 (+7.0%), lockbox DD $32,467 vs $27,506 (+18%), lockbox PF 1.560 vs 1.598; BOOK #429 (#366 base) $1,588,077 (+13.8%), DD $37,738, lockbox $311,161 (+7.4%), lockbox DD $33,292 vs $28,066 (+19%). Clears MAR x1.05 but FAILS the lockbox-DD-within-5% clause on both bases - NOISE #382 is NOISE #304 sized up, so money and lockbox drawdown rise together. Owner call. STUDIES rows 1773-1777 (web v73.908).
- **2026-09-24 - TTIBS CLOSED.** The only honest TTIBS pass, TTIBS #300 (late same-day entry, `TTIBS_1_3.py`, 6/6 gates, LB +$25,359 / PF 1.19 on 39 trades), was added to BOOK #396 as BOOK #419 (same window 2010-06-07..2026-06-30, fixed params, weight 1). Selection net rose $1,483,223 -> $1,625,039 (+9.6%) but max drawdown DOUBLED $36,562 -> $77,832, net/DD fell 40.57 -> 20.88 and PF 1.51 -> 1.42; LB net +$41k but LB drawdown $27,506 -> $49,989. TTIBS #300 is long-only buy-the-dip on NQ with yearly correlation +0.55 to the index and a third of its profit in 2020, so its drawdown lands on the book's drawdown: leverage, not diversification. Standalone it was already weak (LB pre-read four times during design; 2022 negative ex-top-3). Honest next-open TTIBS #291 FAILED (LB -$47,905); TTIBS #161-#201 are void (look-ahead fill). Do not pursue TTIBS further unless a pre-registered regime filter targets the shared-drawdown days, validated on a fresh lockbox.
- **2026-09-14 (book round 55 - CORRECTION; STUDIES rows 1733-1738, web v73.798).** Year by year, #379 does NOT robustly beat #366 (dominates 5/15 full years, loses 4); its ratio lead is the 2020 drawdown. The ORB swap is a coin flip (7 dominate / 6 lose on top of the squeeze swap); the squeeze swap earns more in 13/15 years. **Run #396 = #366 with only `TTMSQZ_3_0_ES30SSOF2` in the squeeze slot: PASS, sel 40.57, held-back $307,096 (PF 1.60) - recommended book.** Slot audit of 7 validated alternatives: none clears. Hourly squeeze tilt on NOISE: beats leverage controls but only 84-87 percent of nulls in the held-back year - parked. `tools/book55*.py`.
- **2026-09-14 — every QQQ signal now names its trade; an exit closes only its own trade (`api/trade_id.py`, `api/cloud_signal.py`, `api/qqq_exec.py`).** Failover sim scenario F, with live evidence: signals.csv rows carried no entry identity on EXIT (ref_time is the exit bar) and the shadow adapter closed whatever lot was open on the leg — at 09:31 ET the live ledger delivered the EXIT of a 2026-09-03 NOISE trade left behind by a replay; no lot was open, by luck. Now a stable trade id (leg + entry bar time in UTC + side, **no price** — Webull and yfinance bars can differ by a cent) is APPENDED as the last column on ENTRY and EXIT rows, stored on the lot and published with each position; an EXIT closes a lot only with the matching id. Anything that does not check out is not applied but counted (today + all time), logged and published under `doc.trade_ids` + the event timeline: EXIT for another trade, id-less rows (old writer — **never acted on, entry or exit**; restart the runner with the adapter), ENTRY bars from another session (replay debris), repeated or overlapping entries. Broker `client_order_id` = `qx` + the trade id without separators + `O`/`C` (e.g. `qxNOISE30420260915T135500ZLC`, 28 chars), identical across hosts, never the wall clock. **Webull's US stock order reference documents client_order_id as max 32 chars, unique per account** — the 40 in `api/webull_orders.py` came from the SDK's Hong Kong docstring, so the sanitizer now caps at 32 (hash fallback 32 alphanumeric); the old wall-clock ids were 36-41 chars and would have been hashed to 39. Restart the adapter onto this code only while flat: a lot opened before trade ids can only close on flat-by/breaker/kill and blocks that leg's entries until then. The engine's own trade memory is keyed by the same id with a one-time re-key of old state — the live state already held one price twin (ENGU-Q 09-09 10:07, seeded at 719.2578, re-emitted as a new ENTRY at 719.2579 after the 12:44 restart). The row cursor is KEPT (a consumed-trade-id set for two-host takeover is still to do) but no longer drops back on a short read of the ledger. The ledger header upgrade is atomic, only ever extends a strict prefix (a newer writer's extra column is never deleted), and rows follow whatever header is on disk. Sim: F "gap closed", A–E unchanged (C now shows identical order ids from both hosts). Smoke tool no longer writes fixture orders into the live broker files.
- **2026-09-14 — the QQQ shadow adapter's cross-host lease is enforced on every path that runs the book (`api/qqq_exec.py` LEASE PROTOCOL, `tests/test_qqq_exec_lease.py`, `tests/test_qqq_exec_serving.py`, `deploy/cloud/README.md`).** `tools/qqq_failover_sim.py` proved the lease was advisory: the runner's fallback thread never checked it and, on Linux, started whenever the systemd unit was REFUSING on it (scenario D); every publish blind-wrote "lease: this host", so one publish made any process the holder; nothing re-checked it while serving; and once writes stopped, the last writer read "ours" while the other host read "stale", so both passed the broker gate (scenario E). Now: (1) one copy per machine — an OS file lock (`SERVING.lock.mutex`) that the loop itself takes, so the unit and the runner's fallback can never both tick; (2) the lease is claimed by Firestore transaction and a refused copy never ticks, leaving `SERVING.lock.standby` so the runner does not start a fallback; (3) publishes still renew it with no extra writes, but a plain overwrite only while our last landed stamp is under 60 s old and a compare-and-set checked it within 30 s (single attempt, so it lands within 8 s) — otherwise the publish IS a compare-and-set, and a compare-and-set that merely ERRORS (spent read quota) falls back to a plain renewal inside that bound, or to publishing the status without the lease field, so the phone tab never freezes; (4) a copy that finds another host's fresh stamp stands down at once (log, event, one ntfy per real loss), including on wake from sleep; (5) broker sends need this host's own stamp under 30 s old, checked at tick start and again before each order. `--once` follows the same rules. The fail-open (shadow book) / fail-closed (real orders) split is unchanged. An independent review found no double-run/double-send path in normal operation; residual, needs both hosts armed: a PC suspended in the instant between deciding a plain renewal and sending it can overwrite a claim made while it slept (fix = server-side precondition renewals). Sim: D and E "gap closed"; A, B, C, F still GAP PRESENT — the lease stops simultaneous trading, not an unsafe takeover (per-host lots/cursor/rails), so the cloud README no longer says two hosts "can never both trade". Mutation-checked: each of 18 deliberate breaks turns a test red.
- **2026-09-11 — a lock held by a DEAD process no longer blocks the adapter's replacement (`api/qqq_exec.py`).** Found by doing it: restarting the adapter after the close, a hard stop skips the cleanup that removes the serving lock, so the lock sat there FRESH — and the replacement launched five seconds later read it, concluded another adapter was already serving, and exited. The adapter was then simply ABSENT, with nothing due to revive it until the next runner boot or the 06:05 wake task. Liveness now needs BOTH tests, because each covers the other's blind spot: the heartbeat age catches a process that died without cleaning up and makes a reused process id harmless, while the process check frees the slot immediately when the holder is gone. If the platform will not say whether a process exists, it is assumed ALIVE and the heartbeat remains the backstop — a false 'dead' would run two adapters, which is the worse error. Three existing tests had been passing on INVENTED process ids and were rewritten to use real ones (the double-run case now spawns an actual second process), because an invented id now correctly reads as a freed slot and would have made them pass vacuously. 13/13.
- **2026-09-11 — a STALE quote is not a BROKEN quote; the adapter was disabling its own pricing before every open (`api/qqq_exec.py`).** Caught on the first clean-coverage morning: the price rail read BLOCKED at 09:54 while a live Webull quote was sitting right there, half a second old. Cause: the quote wrapper treated ANY empty result as a failure and disabled the quote path for 30 minutes. Before the opening auction the newest print is always hours old (logged 62,706 s, then 176 s in the thin pre-open), so the adapter tripped its OWN breaker pre-market and then refused to even ask for a quote until half an hour into the session — and with the NinjaTrader 10-second feed dead again, the price rail correctly concluded it had no price and blocked entries. A call that WORKED but whose newest print is too old now returns a distinct 'stale' result: no price for that tick, path left enabled, logged once per state change instead of every tick. A genuine timeout or refusal still trips the breaker exactly as before. Smoke case 25 pins both directions. 25/25 PASS. This one only showed up because the coverage fixes finally let a morning run clean — each fix exposes the next layer.
- **2026-09-10 (rounds 53-54 - the staleness audit finishes; STUDIES rows 1712-1717, web v73.719).** Two of the three remaining legs were stale. Opening range held the CONTROL `ORB_3_6_C2` (#234) while the crown is `ORB_3_6_R6` (#314) -> run #373 PASS, sel n/DD 41.73 vs 38.18, but held-back -4.5 percent and #314 is WORSE standalone (9.12 vs 10.32), corr 0.899 - thin. Squeeze held `TTMSQZ_3_0_ES30SS20` (#353, $55,912/DD $4,338) while the crown is the COMBINED leg `TTMSQZ_3_0_ES30SSOF2` (#369, $77,105/DD $3,350 - more money on LESS drawdown, same 273 trades) at x3 weight. **Both crowns = run #379, PASS: sel 44.35 vs 38.18, held-back $294,142 vs $289,811, PF 1.548 vs 1.486, nothing given back, no knob tuned.** #378 (squeeze x4) = 47.16 / $316,881, ladder now has an interior best at 4 - recorded as a KNOB TEST, not adopted. `tools/r53_book_orb_leg_audit.py`, `tools/r54_book_squeeze_leg_audit.py`.
- **2026-09-10 — the PC now WAKES for the opening bell (`tools/premarket_ensure.py` + a scheduled task).** Third invalid shadow day in a row, and this one was not a deploy: Windows logged **sleep at 21:42 and wake at 10:18 local**, so the machine slept straight through the 09:30 ET open and the adapter recorded its first tick at **13:20 ET** — nearly four hours of the session gone, with 82 of the day's first 217 ticks stale because NinjaTrader's feed was cold too. `keep_awake.ps1` cannot help: it holds a machine that is already awake and never wakes one, and NOTHING on this box was set to wake it (the only task with WakeToRun was disabled and fired in the evening). New task **EdgeLog premarket wake**, weekdays 06:05 local with *wake the computer* ticked — 06:05 local is before the open in BOTH halves of the year, since Arizona does not shift and ET is local+3 in summer and local+2 in winter. It runs `premarket_ensure.py`, which **never kills anything** (an overnight validate may still be working) and only starts what is missing: the runner fleet if no api.runner process is alive, and the standalone shadow adapter if it is not serving. This is also the plainest argument yet for the cloud box in `deploy/cloud/` — a forward test whose evidence depends on a laptop being awake is not a forward test.
- **2026-09-09 - `import_nt_ohlc.py` converts NT's bar-END stamp at import (the leak that undid the 1m rewrite).** Minutes after the NQ 1m nt_noadj master was restamped, the scheduled 10s-import task appended 4 rows straight back from NinjaTrader's own `NQ_1m.csv` export - still a full minute late - so the repair decayed on its own. The importer now subtracts one bar width for any MINUTE-or-larger series (`_stamp_shift_secs`), and deliberately leaves the 10s and 1s captures raw, because `api/paper.py::_resample` and `backfill_1m_from_10s.aggregate` already read the END stamp themselves - shifting twice would move every rebuilt bar another 10 s. Evidence: the 5,670 NQ 1m rows already merged disagreed with the Databento 1m ETH master on 4,933 of 5,249 overlapping opens as stored, and on 55 shifted back 60 s.
- **2026-09-09 (v73.708) — THE DAY-STAMPING RULE IS NOW A VISIBLE DECISION, not a note in a doc.** Owner: *"make the day-stamping rule an owner decision I can see"*. A trade's calendar day has two defensible answers: the engine truncates a tz-aware US/Eastern index and numpy does that **in UTC**, so a 24h leg exiting at/after 20:00 ET (19:00 winter) books on the NEXT day; the other answer is the ET session day. **Net is identical under both** — only the daily curve moves — but drawdown is measured on the daily curve, so the two disagree about which month holds a book's worst stretch. On the house baseline: **$34,329.21 in 2022-04-27..05-24 (engine) vs $34,903.00 in 2020-02-26..03-25 (session)**, verified by re-running run #337's own legs through the changed engine. `augur_engine/book.py` now scores both and stores `book.day_rule` (rule used, session-day net/drawdown/worst-stretch, `net_differs`, `drawdown_differs`), and the BOOKS row in COMPARE ▸ RUNBOARD prints the second reading beside the first **only when they differ** — never for a day-session book. **NOTHING WAS SWITCHED:** every stored figure is still the engine rule, because changing it would move every recorded book drawdown at once and make old and new runs incomparable. Adopting the session rule is a re-scoring of every stored book, not a patch, and should be decided as that. Guards: two new cases in `tests/test_book_worst_stretch.py` (both rules must report the same NET — a restamp moves trades, it never loses them — and a restamp that carries a loss across a peak must raise `drawdown_differs`; both fixtures open with a gain on purpose, since a curve that only falls has no drawdown by definition), plus `tools/runboard_books_probe.py` extended to pin the row printing it and printing nothing for an older book with no block.
- **2026-09-09 (owner: rewrite it) - the NQ 1m `nt_noadj_eth` master is restamped.** `tools/backfill_1m_from_10s.py --rebuild` (new; `--dry-run` supported) re-aggregates the rows already in `master_b3bf23b6.csv` from the 10s capture with the bar-END stamp, which a normal run cannot do because existing rows win on the merge. 47,589 of 47,694 rows changed; 31 rows the 10s capture cannot rebuild were dropped rather than left behind mixing two stamp conventions; the window is unchanged (2026-06-23..08-13) because restamping and extending are separate operations. Acceptance test against the Databento 1m ETH master on the 12,762 overlapping minutes: open mismatches 11,611 -> 189, high 7,610 -> 20, low 7,585 -> 15, close 11,568 -> 194. The CSV is backed up beside itself (`.bak-<stamp>`) and in `C:\EdgeLog\_ntbackup`, the write is atomic (temp + os.replace) because the runner shares the path, and the registry row count was updated to 47,694. Note the residual: the NT capture and Databento still disagree on ~1.5% of minutes, the same roll-day and thin-minute tail the 5m check leaves.
- **2026-09-09 (ADOPTED - run #366 is the book; STUDIES rows 1699-1703, web v73.706).** Owner adopted the four-leg book, superseding #361: ORB crown + **ENGU-Q #335 crown @0.783 (24h convention)** + TTM structural-stop x3 + **NOISE #304 w1**. Selection n/DD **38.18** vs 30.25, whole **46.11** vs 36.93, held-back level 10.33 vs 10.31, 8/8 slices; net +34% for +6.5% DD. **Cost: PF 1.526 -> 1.486, trades ~2x.** Definition = `tools/queue_book_four_leg.py`. Rejected first: NQDIP 5th leg (38.18->23.37) and the TTM weight ladder (no interior optimum). Caveat: held-back year's top-10 days ~103% of profit in every version.
- **2026-09-09 (round 52 — no fifth leg, no weight change; STUDIES rows 1694-1698, web v73.705).** NQDIP as a 5th leg REJECTED: most uncorrelated leg in the library (−0.003..+0.014) but its own DD $44,824 exceeds the whole book's $36,562 → selection n/DD 38.18 → 23.37. **Uncorrelated is not enough; the leg must be competitive on its own n/DD (NQDIP alone 9.92).** TTM weight ladder improves both stretches monotonically (38.18/40.48/42.78/45.07) with no interior optimum = **metric artifact** (n/DD has no concentration term); at w5 TTM is 26.9% of selection profit from 9% of trading days. Not adopted. **Run #366 stands.** `tools/r52_book_add_nqdip.py`.
- **2026-09-09 — signals that arrive HOURS late are no longer emitted, and the second non-atomic cache writer is fixed (`api/cloud_signal.py`, `tools/qqq_paper.py`).** Caught live, not theorised: after the 12:44 ET runner restart the parallel run re-derived the day and emitted an ENGU-Q **ENTRY stamped 10:07 at 12:44** — two and a half hours old, but still "today", so the existing date test waved it through. An executor cannot take a 10:07 price at 12:44; it would open somewhere else entirely, which is exactly the divergence the parallel run exists to measure. An entry must now be within three bars of `now` (per-leg overridable) or it is recorded silently and counted in `late_skipped`. STALE (entry not even from today, the rolling window re-minting an old trade) and LATE (today, but too old to act on) are counted SEPARATELY — a first cut counted one trade under both, which makes each counter a lie. (The same truncate-then-write in `tools/qqq_paper.py` was fixed independently by another session in d556e35 while this was in the push queue — my rebase dropped my duplicate of it, which is why this entry claims only the late-entry rule.) 6/6 tests.
- **2026-09-09 (v73.704) — the torn-cache race is closed on BOTH writers, and the test guard is split so an empty cache stays LOUD.** (1) The QQQ bar caches are written by two processes; `api/cloud_signal.py` was made atomic at 11:02 in `b1e179e`, but **`tools/qqq_paper.py` writes the very same files and was still doing a plain `merged.to_csv(path)`** — so the race stayed live through that path and fixing one writer closed nothing. It now writes beside the destination and `os.replace`s into place. **`tests/test_qqq_cache_atomic.py`** pins the property for both writers: a failed write must leave the previous cache intact, neither may hand `to_csv` the destination path, and a THIRD writer appearing fails the roster test rather than slipping in unpinned. Mutation-verified on both. (2) My own guard in `tests/test_cloud_signal.py` treated an empty cache as an absent one and SKIPPED — precisely the *"a test failing on data it did not write is worth reading before it is 'fixed'"* caution in the entry below, which I had walked straight into. The conditions are now separated: a **MISSING** cache is an environment fact (a fresh clone has no `C:\EdgeLog`) and skips; a cache that **EXISTS BUT IS EMPTY** is an anomaly — something wrote it and produced nothing — and now FAILS by name. That keeps the signal that caught the hour-long outage while sparing every push from a machine with no cache. (3) **A near-miss worth recording:** the patch that wrote this very entry did `open(path,'w').write(open(path).read()...)`, which truncates the file before the inner read runs, and emptied `BACKTESTING_STACK.md` (2,812 lines). Caught by reading `git diff --numstat` before pushing. Read-then-write is two statements — the same truncate-before-fill shape this entry is about.
- **2026-09-09 (round 51 — the book validates settle rounds 48-50; STUDIES rows 1682-1686, web v73.700).** **#366 FOUR-LEG (ORB 234 + ENGU-Q #335 + TTM SS x3 + NOISE #304 w1) is better: sel n/DD 38.18 vs adopted #361's 30.25 (+26%), whole 46.11 vs 36.93, LB level 10.33 vs 10.31, 8/8 slices; net +34% for +6.5% DD; PF 1.526 → 1.486.** #365 crown swap alone is a WASH (28.77). **CORRECTION: round 48's '11.95 → 28.77' is withdrawn — I reproduced #361 locally with exit-day pooling instead of using the app's own figures; for an overnight leg that halved the measured DD. Baseline against an app-produced run = that run's own numbers.**
- **2026-09-09 (rounds 48-50 — the book was trading the ex-crown; STUDIES rows 1667-1673, web v73.698).** #361 carries ENGU-Q #309 though the crown moved to #335 on 09-08. Crown swap + honest 24h cost: sel n/DD **11.95 → 28.77** (net +22%, DD halved), LB 7.88 → 10.72. Adding NOISE #304 at weight 1: **→ 38.18** (+$347,568 net for +$127 DD), LB 10.33, top-10 31% → 25%. Weight 2 rejected (LB degrades to 9.00 = leverage). All 18 combos scored — four-leg wins, dropping ORB is worse. Both queued (`m3Q0cIr8M5w61Cflqubp`, `pu7lNV62eVgMYwqWS6XU`). Caveat: every version's held-back year is ~103% top-10. `tools/r48_book_crown_swap.py`, `r49_book_add_noise.py`, `r50_book_composition_sweep.py`.
- **2026-09-09 — the QQQ bar cache is now written ATOMICALLY, after a non-atomic write destroyed it (`api/cloud_signal.py`).** `merged.to_csv(path)` TRUNCATES before it writes, so a process killed mid-write leaves an EMPTY file. That is exactly what happened: a runner restart at 13:49 ET caught the parallel run mid-write and `C:\EdgeLog\ohlc\QQQ_1m.csv` sat at **0 bytes for over an hour**, which silently starved the 1-minute ENGU-Q leg of every bar it needs. It surfaced only because the replay tests went red — a reminder that a test failing on data it did not write is worth reading before it is 'fixed'. Now written to a sibling and `os.replace`d, which is atomic on Windows and POSIX, so a reader sees the old file or the new one and never a torn one. Cache rebuilt from yfinance (7,795 rows, 21 sessions, 2026-08-11 to 2026-09-09) and re-verified. The replay tests were also made honest rather than lucky: they snapshot the live cache ONCE per session and validate each copy parses, and they replay the newest FINISHED session rather than today's still-growing one, because the parallel run now rewrites that cache every 30 seconds.
- **2026-09-09 — the QQQ shadow adapter runs as its OWN process; fleet restarts no longer cost it uptime (`api/qqq_exec.py` serve/ensure_standalone, `api/runner.py`, `deploy/_run_qqq_exec.vbs`, `tests/test_qqq_exec_serving.py`).** THE PROBLEM, measured: the adapter rode as a thread inside the job runner, so its uptime was a property of the busiest process on the box — roughly 30 concurrent sessions restart the fleet all day to load code, and on 2026-09-09 that booted the adapter **eight times between 09:47 and 13:49**. The day held ~94% tick coverage against a **95% readiness bar**, i.e. the forward trial was being failed by DEPLOYS, not by anything wrong with the adapter, and the same thing had already invalidated 09-08. Uptime cannot be a property of the busiest process. Now the adapter runs detached and `api/runner.py` calls `ensure_standalone()`, which leaves a live adapter strictly alone and launches one only when none is serving — a fleet restart becomes a no-op for the shadow book, while a dead adapter is still auto-revived by the next runner boot. Liveness is a **heartbeat file rewritten every tick**, never a bare pid: a pid can be reused and a hard kill never cleans up, so a lock older than 120 s frees the slot. Worst case on a race is a brief double-tick, which is harmless — both writers compute the same state from the same fills. The in-runner thread stays as the fallback when the launcher is missing. 11 new tests + smoke 24/24. DEPLOYED AFTER THE CLOSE deliberately: installing it mid-session would have cost the very coverage it exists to protect.
- **2026-09-09 (v73.692) — PAPER LEG `ETFBOOK_332` RETIRED, the day it was added and before it wrote a row.** Owner: *"retire the ETF legs from the shadow book"*. The seven ETF dip legs went through leg-level Auto-Validate that morning (runs #354-#359, six jobs — two of the seven differ only in a bool the search always explores) and **6 of 6 FAILED, every one on walk-forward efficiency (0.050-0.332 against a 0.5 bar)**, with **not one crown matching the cell the leg carried**. They are not noise cells (t 2.41-5.57, all p < 0.02, `causal` and `plateau` clean on all six) — they are real in-sample edges that do not survive being re-fitted forward, which is exactly the question a forward test is there to answer, so shadow time on them buys nothing. **ARCHIVED, not deleted**, per the house convention: the entry comes out of `PAPER_LEGS`, the `LEG_SOURCE` provenance block stays (with a RETIRED caveat) so any report naming the key resolves, and the board keeps it behind SHOW ARCHIVED with colour and label intact. **One switch, not two:** the runner hook in `api/runner.py` stays wired and `etf_book_shadow.maybe_nightly_update` now no-ops when the key is absent from `PAPER_LEGS` — registration is the only switch, and re-adding the entry restores the whole path. This is load-bearing because that hook APPENDS Yahoo bars to the frozen daily masters every evening, so a half-retirement would have been invisible (no row is written anywhere a person looks). Two new tests in `tests/test_etf_book_shadow.py` pin the hook being off, the re-enable path, and the surviving provenance; deleting the check makes them fail. `BOOK.md` §10d.
- **2026-09-09 — CORRECTED. This entry originally said main went red because the QQQ 1-minute cache was empty, and speculated the cause was the switch of that feed from yfinance to Webull. The speculation was wrong.** The cause is the one established in the entry above: a runner restart caught `api/cloud_signal`'s writer mid-write, and because `merged.to_csv(path)` truncates before it fills, the cache sat at 0 bytes for over an hour — starving the 1-minute leg, not just the tests. Nothing to do with Webull. What the entry got right is narrow: `HAS_REAL_CACHE` tested `os.path.exists` only, so the skipif sailed past a zero-byte file and both replay tests died inside `pd.read_csv` with `EmptyDataError`, blocking every push in the repo behind a message that named pandas rather than the cache.
- **2026-09-09 (round 47 — CORRECTION to round 46; STUDIES rows 1642-1647, web v73.688).** The ENGU-Q ES leg's 'recency pass' was ten trades: ES 2024-2026 top-10 = 123.6% of net, ex-top-10 −$19,909 / PF 0.862, half the profit from 4 trades; 2010-2023 top-10 = 102.2%, ex-top-10 −$2,656. **Only 3 of 16 years survive removing that year's 3 biggest trades.** NQ crown by contrast keeps +$262,415 / PF 1.302 ex-top-10 — but its own 2024-2026 stretch is tail-driven (top-10 106.3%, one trade $91,152). Validates left running; WF+lockbox cannot detect concentration. **Process fix: concentration test runs per CLAIMED STRETCH, not only whole-window.** `tools/r47_enguq_es_recent_concentration.py`.
- **2026-09-09 — the 10-second NQ feed is now WATCHED (`api/nt_heartbeat.py`, `tests/test_nt_tick_feed.py`).** The blind spot that let 2026-09-09 happen: NinjaTrader was healthy all session (bridge up, all three strategies Realtime, fills.csv fresh, the dead-man's switch green) while its 10-second chart export had been dead since 08:39:40 ET — of that morning's 3,087 exported bars, 3,075 were the chart's historical backfill and only 12 were live prints, the indicator having lost its tick subscription 24 s after the strategies were enabled. NOTHING NOTICED, because every existing watcher asks a different question: nt_recover.ps1 asks about STRATEGY state, the bridge asks whether NinjaTrader is running, and nt_heartbeat asked whether the BRIDGE is still publishing. A chart indicator that stops writing answers yes to all three. Cost is real, not cosmetic: with no live NQ price the QQQ shadow cannot mark an open lot, so an EOD flatten prices the exit at the ENTRY price and writes a fabricated round trip into the forward record. Now `evaluate_tick_feed` (pure, so the real outage is replayed in the tests) runs on the existing 300 s heartbeat timer, publishes a `tick_feed` block on meta/nt_alert, and pages ntfy ONCE per outage — the flag latches so a long outage does not spam and clears itself when bars resume. Watched only 09:30-16:00 ET on session days: NQ trades nearly 24/5 but the harm is concentrated where the shadow mirrors, and a 03:00 page for a feed nobody trades off teaches people to ignore pages. 8/8 tests. NOT a restart: the feed had already self-healed at 11:28 ET and has been continuous since, so there was no fault to restart.
- **2026-09-09 (round 46 — ENGU-Q ES leg vs the recency and cost tests; STUDIES rows 1622-1627, web v73.676).** Recency: PF 1.244 (2010-2023) → **1.585 (2024-2026)** at $30/RT, $65.84 → $357.31 a trade; NQ leg 1.611 → 1.857. Opposite of r43-45's NOISE finding. Cost: $98.48/trade at $30/RT = 3.3x cost (NOISE-on-ES was <2x and was closed). Marks: bigger trades not more (236 vs 1,871), n/DD 2.52 recent vs 5.49 older, 2015/2016 losing years, 2026-to-June flat. **Second ES validate queued at 0.60/RT (KLxmi3KXnSK7n0KV6D1Z)** per the round-42 rule. `tools/r46_enguq_es_recency.py`.
- **2026-09-09 — the cloud signal engine reads Webull bars, yfinance demoted to fallback (`api/cloud_signal.py::_fetch_webull`).** With Nasdaq Basic non-display claimed, QQQ bars now come from the exchange's own consolidated feed instead of a free scrape: newest 1-minute bar arrives about a second behind the print, against yfinance's measured ~38 s median for the newest CLOSED minute. Only the TAIL is fetched (200 bars) because the API caps a request at 1200 while the rolling window wants tens of thousands, so history stays in the on-disk cache and this tops it up — the same shape the yfinance path had. RTH only, matching prepost=False, since the crowned configs are regular-session configs. **The seam was measured, not assumed:** across 200 overlapping 1-minute bars the two sources' closes differ by a median of $0.0000 and a maximum of $0.0206 (0.29 bp), with one bar over a cent — so a cache holding rows from both has no step in it. yfinance still runs when Webull is unavailable, and logs when it fires rather than switching silently. Note the SDK trap again: `MarketData(api)` does not authenticate, `ClientInitializer.initializer()` must be called first or every request is a silent 401.
- **2026-09-09 — the QQQ shadow prices off a REAL Webull quote; the quote path had never worked (`api/qqq_exec.py`).** The owner claimed the free **Nasdaq Basic non-display** tier (Open API Advanced Quotes at webullapp.com, now "Free Authorized" to 09/2027, profile reads Non-Professional Subscriber), and fixing two independent bugs turned it on: (1) **no token** -- `MarketData(api)` does not authenticate; only `ClientInitializer.initializer()` mints the x-access-token, and the SDK runs it inside TradeClient/DataClient, never inside MarketData, so every request went out unauthenticated and came back 401 INVALID_TOKEN, which the function swallowed as "no entitlement". The 403 MARKET_DATA_NOT_SUBSCRIBED seen while spiking was a RED HERRING -- that script built a TradeClient on the same client first. (2) **wrong shape** -- `get_snapshot` returns an HTTP response whose payload is a list of plain dicts, and the code read it with `getattr`, which returns None against a dict; the timestamp field is `last_trade_time` in epoch MILLISECONDS, not any of the three names it looked for. Either bug alone made the path unusable, so shadow fills have ALWAYS been priced off the NQ:QQQ ratio. Verified live: QQQ 715.77, age 1.5s. This removes the ratio dependency AND the dead-10s-feed exposure, since resolve_price consults the quote first and both marking and closing go through it. The price-feed rail added earlier today is therefore RELAXED to fire only when BOTH sources are gone (quote probe cached 60s, only consulted when the NQ feed is already stale). Smoke case 24 now covers all three states: both dead = blocked, NQ back = clear, and quote-alive-with-NQ-dead = trade allowed and priced off the quote. 24/24 PASS.
- **2026-09-09 (round 42 — NOISE ES branch CLOSED; STUDIES rows 1577-1583, web v73.658).** Cost ladder: ES 3/9 cells clear (only at $15/RT); at $25/RT every bar size fails PF (1.20-1.24); NQ 9/9 clear at up to $20.66/RT. Per-trade edge ES $44-57 vs NQ $101-149 — ES pays <2x cost (house rule is 2x), NQ 6-14x. **Validate #344 WEAK** (WF 8/8, PBO 0.675 = likely overfit selection, LB 184 trades PF 1.134). Instrument problem, not config problem. Cross-instrument evidence survives. **Any future ES validate must be queued at >=0.50/RT, not 0.30.** `tools/r42_es_config_cost_ladder.py`.
- **2026-09-09 — live gate on the 10s fix, a feed outage, and the parity rows that were never lost.** The LIVE gate service was restarted onto the bar-END resampler and verified with `GET 127.0.0.1:8392/gate/health`, which reports the git SHA THAT PROCESS loaded (079c7f2 -> 93c06a9) - the only honest check for "pushed" vs "running". Separately the 10s export died twice: both capture paths (the chart indicator writing `C:\EdgeLog\ohlc` and the headless add-on writing `C:\EdgeLog\ohlc_addon`) stop at the SAME bar ~3 minutes after a connect, so a missing chart is never the diagnosis on its own. A restart that RESUMED the NT session did not fix it; the restart that went all the way to the login window and re-authenticated did (feed continuous since). The add-on cannot be switched off by editing `C:\EdgeLog\config.txt` - with no symbols listed it falls back to hardcoded ES+NQ 10s. Finally, restarting NT drops EdgeLogENGUQPAR / EdgeLogORBPAR out of the Control Center grid, which looks like deletion and is not: all six EdgeLog rows keep their DB records and links, so `--add-parity-rows` is a no-op; what is missing is the workspace entry hosting each row on its chart. New `tools/nt_reconfig.py --host-parity-rows` writes those two entries (ENGUQPAR on the NQ 1m chart, ORBPAR on the NQ 5m chart, matched exactly once by its primary series so it cannot mis-bind), NT stopped, XML validated, idempotent. Reachability afterwards is `GET /strategy/check?name=EdgeLogORBPAR`, not `/strategies` - the latter lists each account's ENABLED instances only.
- **2026-09-09 — the PARALLEL RUN starts collecting (`api/cloud_signal.py::cloud_signal_thread`, wired into `api/runner.py`, primary only).** The signal engine now steps through every regular session inside the runner, so QQQ-bar signals accumulate beside the NinjaTrader-mirrored shadow book over identical sessions. That side-by-side record IS the evidence the "drop NinjaTrader entirely" decision needs, and it is worth nothing unless it is collected live rather than replayed after the fact. 30 s cadence rather than the CLI's 20 s: each step may hit yfinance once per timeframe before step()'s cheap no-new-bar short-circuit can skip the engine, and a 1-minute leg cannot gain a bar faster than once a minute. SIGNALS ONLY -- the module imports no broker SDK and has no order path; nothing downstream of signals.csv exists. Verified live for 45 s: all three legs cold-start seeded without emitting, heartbeat written, clean stop. Two days of this beside the shadow book answers whether QQQ bars alone reproduce what the NQ fills produce.
- **2026-09-09 (v73.638) — THE RISK CLAUSE IN EVERY BOOK BAR WAS INERT; audited across all six adoptions, replaced, and the engine now supplies the evidence.** Every book bar this shop has written read *"annualised MAR at least X above the adopted book, at a **whole-run drawdown** within Y percent of it, lockbox at least as large."* A book's whole-run drawdown is ONE stretch of tape, so a candidate leg that took no trades inside that stretch cannot move it at any weight: BOOK runs **#336, #337 and #341 all record $34,329.21 to the cent** while #336/#341 carry three ES contracts #337 does not have at all. **Audit (`tools/book_clause_audit.py`, log `tools/data/book_clause_audit.txt`, csv alongside):** baseline #337's stretch is **2022-04-27..05-24, $34,329** (ENGU-Q 92%, ORB 8%). **#336 vs #337** (TTM adopted) old PASS / **lockbox clause PASS** — LB DD $26,235 vs $33,112, it *improved* the number the weak clause could not see. **#341 vs #336** MISS under both (MAR x1.049 vs the 1.05 bar — the recorded near miss, unchanged). **#342 vs #336** old PASS / **lockbox PASS** (LB DD $26,098 vs $26,235). All three TTM legs traded the incumbent's worst stretch **zero** days, so the drawdown clause could not have failed in any of them. **Weak-edge ETF book #332/#338**: own stretches 2011-05-30..08-16 $73,194 (NQDIP 43%) and 2011-06-15..08-16 $43,818; its r25 bar carried no drawdown-vs-incumbent clause at all, and its stack test is **not** inert — all seven ETF legs trade the r25 champion's 2020 stretch (they are dip-buyers and that stretch is a crash). The stack it MISSED (+15% MAR) misses harder on the lockbox: #332 risk-matched MAR x1.232 at lockbox drawdown **x1.891**, #338 risk-matched x1.334 at **x1.737**. **NASDAQ WF book (B11)** is the exception and the one to re-read: 6 of its 8 legs DO trade the champion's worst stretch (2020-02-25..03-25, $71,773; they add −$52k of the stacked −$123,925), so its **+15% MAR bar was doing real work** — but the stack lifts whole-run drawdown **+73%** and last-12-months drawdown **+104%**, and it **MISSES a lockbox-drawdown clause outright**. **NOTHING IS RETRACTED HERE** — the evidence is put in front of the owner. **REPLACEMENT CLAUSE** (canonical wording in `tools/book_dd_attribution.bar_text()`, doctrine in `BOOK.md` §10): MAR x1.05 + **LOCKBOX drawdown within 5%** + lockbox net >=, with the whole-run drawdown reported as a **check that says out loud when it is inert**. The lockbox drawdown is the right number because it moves with weight (TTM $26,235 → $29,128 from three contracts to six) and is measured on unseen tape. **Engine:** `augur_engine/book.py` now writes `book.worst_stretch`, `book.worst_stretch_lockbox` and `book.inert_legs` on every book result — dates, depth, each leg's dollars and trading days inside the stretch, rows summing to the drawdown to the cent (span is the days strictly AFTER the peak). COMPARE ▸ RUNBOARD's BOOKS table prints the stretch under each book's legs and flags absent legs in yellow. New: `tools/book_dd_attribution.py` (library + CLI `--run N --vs M --new-legs K`), `tools/book_clause_audit.py`, `tests/test_book_worst_stretch.py`. STUDIES block `bookclause`.
- **2026-09-09 — OPEN ITEM found by that audit: two day-stamping rules disagree about a book's drawdown.** The recorded finding put the baseline's worst stretch at **2020-02-21..03-25 / $34,903**; the stored BOOK runs record **2022-04-27..05-24 / $34,329**. Same legs, same params, same master, **net identical to the cent**. `augur_engine/book.py` pools with `np.asarray(index, dtype="datetime64[D]")` and numpy truncates a US/Eastern index **in UTC**, so a 24h leg's trade exiting at/after 20:00 ET (19:00 in winter) is booked on the NEXT day; the TTM driver used the ET calendar day. **227 of the ENGU-Q ETH leg's 1,179 exit days** move between the two rules; day-session legs are unaffected. Neither rule is the CME trade-date convention (18:00 ET roll). **Not changed** — every stored book run is on the engine's rule and moving it would move every recorded book drawdown at once; `tools/book_dd_attribution.leg_daily(..., day_mode="session")` computes the other on demand and section 0 of the audit prints both. The inert-clause finding holds under BOTH rules (the TTM leg traded the stretch zero times either way). **Owner call.**
- **2026-09-09 — PRICE-FEED RAIL: the QQQ shadow never opens a lot it cannot mark (`api/qqq_exec.py::_check_px_feed`).** Found live on day 3 of the trial. NinjaTrader was perfectly healthy (bridge up, all three strategies Realtime, fills.csv fresh) while its 10-second NQ chart export died 40 seconds after the strategies were enabled at 08:39 ET and stayed dead all session -- of that day's 3,087 exported bars, 3,075 were the chart's historical backfill and only 12 were live. The existing `feed_stale` rail watches the FILL feed and says nothing about the PRICE feed, and the two die independently. With no live NQ price `_latest_nq_px` correctly returns None, but `_close_all` then falls back to `lot["nq_entry_px"]`, so an EOD flatten would have written an exit AT THE ENTRY PRICE: a fabricated round trip with a plausible P&L that fails NT parity and poisons the readiness evidence the go-live decision rests on -- the same 2026-09-03 corruption v73.469 fixed, reached by a different road. Now a dead price feed BLOCKS new entries (exits are deliberately still allowed, so an open lot always gets its chance to close); the signal is recorded FIRED + REFUSED with a `px_feed_down` event, and `px_feed_stale` publishes to the tab. A refused signal is explainable evidence; a mispriced trade is corrupt evidence, which is worse than none. `tools/qqq_exec_smoke.py` case 24 covers both directions (blocked while dead, clears when the price returns); the harness also had to be given a live price stub, which revealed that every case in that file had been silently exercising the DEGRADED marking path. 24/24 PASS. NOT FIXED HERE: the dead chart export itself needs NinjaTrader restarted, which is the owner's call.
- **2026-09-09 — free market data IS fast enough for a 1-minute loop, measured (`tools/qqq_feed_latency_probe.py`).** The cloud plan rests on running the crowns off FREE data, because Webull's OpenAPI answers 403 MARKET_DATA_NOT_SUBSCRIBED for BOTH stock and futures quotes on the owner's keys. Live regular-hours probe of yfinance QQQ 1m: newest CLOSED bar **median 36 s / p90 58 s** old, call latency 0.41 s median / 1.16 s max, 0 errors in 21 samples -> PASS against the pre-registered p90 <= 90 s bar. Note yfinance returns the still-FORMING bar (raw age reads negative); a live consumer must drop that last row, and the tool's `--summarise` does the +60 s conversion so the column and the verdict cannot drift apart. The owner's old "1-2 min delay" objection was about clicking a browser paper account, not about the data. Yahoo's NQ=F is ~10 min delayed and stays unusable for signals, so this verdict covers QQQ SHARES only. Same probe caught the NT 10-second NQ feed sitting ~96 min stale (see PAPER_TRADING notes).
- **2026-09-09 — `list_masters()` no longer fails a checkout that has no master registry (`augur_engine/data.py`).** `optimizer_history.db` is untracked, so every git WORKTREE starts without one and `sqlite3.connect()` silently creates an EMPTY database whose first query raises `no such table: csv_files`. `tests/test_etf_book_shadow.py` already had the right intent — `find_master(...) is None -> pytest.skip("not registered in this checkout")` — but the bare `DatabaseError` escaped before the guard could run, so those 6 tests HARD FAILED in every worktree, failing the pre-push engine gate and blocking `tools/wt.py ship` for every session (hit while shipping the cloud signal engine). A missing table now returns `[]`; any other database error still raises, and a checkout WITH a registry is completely unaffected. Suite in a bare worktree: 1193 passed, 93 skipped, 0 failed.
- **2026-09-09 — cloud SIGNAL ENGINE (`api/cloud_signal.py`, `tests/test_cloud_signal.py`).** Runs the three current crowns (ORB_R6 #314, NOISE_SBS_V90 #243, ENGUQ_335) directly on QQQ RTH bars and emits discrete idempotent ENTRY/EXIT signals to `<EDGELOG_HOME>/cloud_signal/signals.csv` — the forward half of what `tools/qqq_paper.py` does as a blotter, and the piece a future Webull execution adapter would consume. NO ORDER CODE: no broker SDK is imported anywhere in the module. Portable paths via `EDGELOG_HOME` (defaults to the existing `C:\EdgeLog` cache on this PC, `~/edgelog` on the cloud box). `--replay DAY` re-runs a cached session bar-by-bar, `--once`/`--loop` run live, heartbeat file for the watchdog. **Two defects found and fixed by replaying real data, both of which would have fired live orders:** (1) COLD START — the engine recomputes each leg's whole rolling window, so the first call discovered every trade in the last 60 sessions and emitted them as fresh ENTRYs (22 for ORB alone); a first call now absorbs the history silently and writes one non-actionable SEED row (same rule api/qqq_exec.py needed for NT fills in v73.459). (2) STALE ENTRIES — trades at the rolling window's LEFT EDGE are not identity-stable (ORB_3_6's volume-pace reference is a 20-bar nanmean that is an empty slice at index 0), so old trades shift entry price by a hair, get a new key and re-fire: a 2026-09-04 replay emitted ENTRY events dated **2026-07-01**. Entries outside today's session are now recorded silently and counted in `stale_skipped` (9 suppressed on ORB across two replayed days); their exits are suppressed with them, while exits of trades whose ENTRY did fire still emit on a later day. Clean two-day replay: 09-04 = 3 SEED + 6 real signals, 09-08 = 2 EXITs, rerun = 0 new events. NT comparison for 09-08: NinjaTrader produced ZERO strategy fills that day (it was down until 09:43; the only fill is the owner's manual MES trade) while the QQQ-bar engine still signalled — the concrete case for taking NT out of the loop. KNOWN LIMITATION, flagged loudly in the module docstring: ENGUQ_335 is an ETH (24h futures) config and QQQ has no overnight tape, so its `regime_len` block means ~3.6 calendar days on the tape it was validated on but 10 SESSIONS here, and its absolute $0.50 min-risk / 0.25 ATR floors were calibrated for NQ's price level, not a $700 share — that leg's signals are exploratory, not evidence-backed, until an RTH re-validation. ALSO: `tools/qqq_paper.py`'s ORB leg moved from #234 to the current #314 crown (dict key `ORB_234` kept because index.html hardcodes it; now ungated, matching api/paper.py's own ORB_R6 leg) — its MODEL blotter therefore mixes pre- and post-swap rows with no config marker; the owner's real forward record is the SHADOW book, not this one.
- **2026-09-09 (round 41 — the CORRECTION to rounds 37/38; STUDIES rows 1445-1453, web v73.623).** **(A) 2 minutes is nobody's best bar.** Bar ladder 1/2/3/5/10m on ONE source tape (1m RTH resampled in-file, gated against the registered 5m master to 0.11% on net), at 0.533 AND 0.783. Net-over-drawdown at 0.783: crown geometry **10m 23.6 / 3m 23.4** / 2m 19.4 / 5m 16.1 / 1m 7.4; #334 champion 3m 10.9 / 2m 10.6. Rounds 37/38 ranked on R/YR, which is EV-R x trades-per-year and therefore rises MECHANICALLY when a shorter bar chops one edge into more trades. **(B) The 2m cells are the 5m crown re-expressed, not a second edge:** #334 champion vs #243 crown = 81% shared trade days, **98% same direction**, daily corr 0.811 (0.909 shared), and **−$93,445 earned on the days the crown is flat**; the cost-robust corner is 92% / 98% / 0.711 with only $15,177 (6%) away from the crown. Pooled gains (23.65 vs 18.17) are averaging, not diversification. **DECISION: no 2m leg to paper; nothing moves.** **(C) The one good result is a GEOMETRY:** the r38 cost-robust corner clears the house bar in ALL TEN ladder cells (5 bars x 2 costs) — a plateau across the bar dimension; on the crown's own 5m bar n=1,956 / PF **1.563** (crown 1.418) / DD $12,737 (crown $18,425) / 8-of-8 / $104 a trade → a REPLACEMENT candidate for the crown geometry, never an addition. **(D) METHOD BUG, fixed in two files:** `pd.concat` on date-indexed daily series returns the union UNSORTED, so a pooled `cumsum` walks the calendar out of order — caught by the bound that a sum's DD cannot exceed the sum of its parts' (read $222,772 against a $31,517 bound; sorted $23,899). `tools/gapgo_vs_orb_overlap.py` shared the bug (10.82 unsorted vs 10.63 sorted). Both now sort and assert. Harness `tools/r41_bar_ladder_overlap.py`. Full doc: `NOISE.md` round 41.
- **2026-09-09 (73.622) — 10s bar-END stamp fix in every consumer.** The NinjaTrader 10s export stamps rows at the bar END; `api/paper.py::_resample` now reads it that way by default (`stamp="end"`, bucket on `time-1`), so the LIVE gate bouncer (`api/gate_live.py`), the candle window (`api/bars.py`, which also picks the 10s file by instrument now) and the paper tail rebuild the masters' bars: vs the Databento 5m masters since July, NQ open mismatches 3,307→136 of 3,659, ES 1,883→19 of 3,653 (`tools/diag_10s_stamp.py`, unit test `tests/test_resample_stamp.py`). `tools/backfill_1m_from_10s.py` had the same shift: the NQ 1m `nt_noadj_eth` master it extended is written shifted (42,055 backfill rows by 10 s; the 5,670 rows imported from NT's own 1m export by 60 s — NT stamps that at bar end too) and disagrees with Databento on 11,611 of 12,762 overlapping opens. Aggregator fixed, master NOT rewritten (owner call; nothing reads it — `find_master` prefers the Databento file). Running bouncer (pid started 05:02) keeps the old code until `C:\EdgeLog\_gate_server.bat` is restarted. Two worktree-only test landmines surfaced by the pre-push engine gate and fixed in the same push: `tools/queue_guard.py` inserts the shared desktop checkout at `sys.path[0]` on import, so any test importing it makes every LATER `import api.x` resolve to the OTHER checkout (this suite was silently testing the shipped `_resample`) — `tests/test_resample_stamp.py` now pins its own tree; and `tests/test_etf_book_shadow.py` called `find_master` outside its skip guard, which RAISES (not returns None) in a worktree because sqlite creates an empty optimizer_history.db on connect — it now skips, and still passes in the desktop checkout that has the registry.
- **2026-09-09 (round 40 — NOISE ES bar-size travel + cross-instrument OOS; STUDIES rows 1422-1429, web v73.617).** ES 5m and ES 15m searches converged on the SAME 15-knob leader; it clears on ES at 5m/15m/30m (n/DD 10.8/13.2/8.3) and fails at 60m (top-10 67%). Run on NQ (never seen during selection) it clears at all four bar sizes (PF 1.43-1.50, n/DD 14-20, top-10 24-31%) and TIES the crown card on the crown window (n/DD 19.6 vs 18.3, 8-of-8 vs 7-of-8, less net). **Live-leg caution: in the last 12 months both configs are tail-driven (top-10 214%/176%, ex-top-10 negative).** Harness lesson: read params from the search CSV, not a truncated console line. `tools/r40_noise_bar_travel.py`, `tools/r40b_es_selected_vs_crown.py`.
- **2026-09-09 (ENGU-Q R4/R5 and the frontier books — FOUR JOBS QUEUED).** `tools/enguq_search_r5.py` walked the five settings this campaign never touched; only the **entry limit depth** improves the crown (0.85 ATR: MAR 0.92→1.08, net +0.7%, 2022 $7,340→$17,489, tail unchanged) → **ENGUQ_1M_ETH_R4_1_0.py**, job 4Oy6lWgZ. It also found a SECOND dead knob: `er_len` has no power because `er_th`=0 switches its gate off, so with `buf_atr` that is two of fourteen knobs a full-discovery validate searches for nothing. `tools/r4_plus_cap.py` then asked whether the deeper limit and the hold cap stack — **they do**: limit 0.85 + cap 9,660 gives **MAR 1.16, top-10 43.0%, index correlation +0.27, 2018 +$13,022, 2022 +$24,796, 151 held-out trades, longest hold 15 days**, 2 of 16 cells clearing and they are neighbours. **This is the FIRST ENGU-Q config to PASS the enforced tail bar** (guard: PASS/PASS, top-10 45%, corr +0.273, reload 152 vs continuous 151); its held-out year beats the crown (151 tr / $99,997 / R-YR 66.8 vs 118 / $88,380 / 57.5) for 7.5% of net → **ENGUQ_1M_ETH_R5_1_0.py**, job CtGFgEBu. Parity exact against both the crown and the R3 parent. **BOOKS:** `tools/frontier_repair.py` — the NOISE R/YR leg goes NEGATIVE under proportional treatment (−$6,317), so the pair was repaired with ORB #234 (pooled MAR 1.38→1.69, index corr +0.04, job LUxH9euN). **Then `tools/frontier_trio.py` overturned that narrative:** the incumbent trio CARRYING that leg has the best pooled MAR measured (1.84) — a leg can be fragile alone and still earn a book slot, because a book wants uncorrelated P&L, not a standalone edge. The **QUAD** (that trio + ORB) was queued instead (job y0Yci2ca): net $1,439,831, MAR 1.80, top-10 31.3%, **index correlation −0.11**, 2018 +$89,996, 2022 +$218,051 — +35% net over #324 for 2% less MAR and a deeper drawdown, stated as a trade-off.
- **2026-09-09 (tail-adjusted board read — `tools/tail_adjusted_board.py`).** Every crowned leg re-scored on EV R / R-YR after deleting its biggest winners. **Tail dependence is universal: all five legs lose 40-51% of EV R to ten trades** (ENGU-Q crown 51%, capped 44%, NOISE R/YR frontier 42%, NOISE crown 40%, ORB #234 43%) — across three families and both session types, so the owner's challenge applies in degree to the whole board, not just ENGU-Q. The fixed-ten EV R ranking is **unchanged**; on R-YR the only swap is **the capped ENGU-Q overtaking the live crown (31.8 vs 29.7)**, the first support for the hold cap not derived from the concentration reads themselves. **THE IMPORTANT CORRECTION: the fixed-ten rule flatters high-frequency legs** — ten trades is 0.55% of the crown's 1,831 but 0.18% of the frontier's 5,548. Under equal treatment (top 0.5% of each leg's OWN trades) **NOISE_1_2_RYR collapses from EV R 0.279 to 0.004** (28 of 5,548 removed) and falls from third to LAST, behind ORB; everything else barely moves (crown 0.263, capped 0.173, NOISE crown 0.089, ORB 0.078). That leg passed the existing fixed-ten concentration check (top-10 43%) only because it trades so often — the check's fixed count cannot see the shape at that sample size. **Use the proportional column for any concentration read on a high-frequency leg.** Board read, nothing queued.
- **2026-09-09 (horizon mixing CLOSED — `tools/r3_horizon_weights.py`).** The 1:1 two-horizon book tied, so the whole weight curve was walked (capped-leg weight 0.0 → 1.0). **MAR falls monotonically — 0.97 / 0.97 / 0.96 / 0.96 / 0.95 / 0.94 / 0.93 / 0.93 / 0.87 — so no mix comes near the +5% the bar required, and nothing is queued.** All weights are 17/17 on positive years; every interior weight shares the same $38,687 drawdown (one episode dominates the curve, another reason not to read small MAR gaps as signal). **The one thing worth keeping is a PARETO point, not a win: at 20% capped weight MAR is 0.97 — identical to the uncapped crown — while top-10 concentration falls 55.6% → 46.8% and index correlation 0.48 → 0.47.** Free tail reduction, no edge improvement; the bar deliberately did not reward it. Practical shape is 4 uncapped contracts to 1 capped, so it needs size to express. Owner call about what the book is FOR; it does not promote itself. Horizon mixing does not need re-opening as a route to risk-adjusted return.
- **2026-09-09 (two horizons of the same edge — NEAR MISS, nothing queued — `tools/r3_two_horizon_book.py`).** The uncapped crown and the hold-capped crown are one entry with two exit horizons and fail in different places, so they were pooled 1:1 as a book. **4 of 5 pre-registered bars pass and the one that decides fails: pooled MAR 0.95 vs the better leg's 0.97.** Everything else is good — pooled net $1,179,039, DD $77,374, **top-10 36.2% (the lowest measured anywhere on this family**, vs 43.8% capped / 55.6% uncapped), index correlation +0.45, 2018 +$24,111, 2022 +$7,966, 17/17 positive years, and the legs share only **+0.41** of their daily P&L so the second contract is NOT leverage. But net doubles and so does drawdown ($39,200 → $77,374), so it buys size and a smoother profit distribution, not edge per unit of risk. Bar 1 required BEATING the better leg exactly so a tie could not be talked into a promotion — nothing queued. If a future session wants this book the honest argument is concentration, not MAR. **BUG WORTH KEEPING:** the first run pooled with `sum(Series)`, which NaNs every day only one leg traded and then collapses those to zero — it reported net $374,374 (less than either leg alone, impossible) with a convincing all-FAIL verdict. Check pooled net == sum of leg nets before reading anything off a pooled series.
- **2026-09-09 (the hold cap is a MECHANISM on NQ, plus a dead knob — `tools/r3_cap_generality.py`).** 19 entry variants (trendline lookback, trend filter, breakout buffer, regime filter, efficiency threshold, decisiveness), each run capped and uncapped, scoring the DIFFERENCE the cap makes. **All four pre-registered bars clear: concentration falls in 95% (bar 80%), index correlation falls in 89% (bar 70%), median MAR cost −5.5% (bar −15%), held-out sample rises in 100% (bar 70%).** So the cap is not fitted to the crown's entry — which sits alongside, not against, the ES finding: general across entries on NQ, worse than useless on ES. **`buf_atr` IS A DEAD KNOB**: 0.2 / 0.4 / 0.5 give byte-identical results because the entry gates on `buf_atr * ATR` and then on `(close − trendline)/ATR >= min_brk`, and with min_brk 1.6 against a 0.0-1.0 buf_atr fence the second gate always dominates — buf_atr can only bind if a search drives min_brk below 1.0. Any historical "plateau across buf_atr" in this family is illusory breadth, and full-discovery validates are spending a search dimension on it; narrowing the knob is an owner call, not done here. **The efficiency-ratio gate at 0.15 is the best correlation reducer measured (+0.19 uncapped vs the crown's +0.51) and is still NOT a candidate** — absolute reads are weak (n 1,064, $396,100, MAR 0.40, top-10 70.3%, 77 held-out; capped MAR 0.53 / 61.3% / 90) and er_th 0.25 is an outright artifact at top-10 100.7%. Recorded so it is not re-discovered and mistaken for a lead. **Nothing new queued: nothing cleared the bar.**
- **2026-09-09 (the sweep leader DIES on the travel test — `tools/r3_es_travel.py`, `tools/r3_scalefree_halves.py`).** The cap-9,660 / trail-2.0 / breakeven-1.5 leader that beat shipped R3 on every NQ read is **DROPPED**. Taken unchanged to ES 1m 24h (0.40 / $50, nothing re-fitted) it is WORSE than R3 on three of four reads — PF 1.183 vs 1.215, MAR 0.33 vs 0.34, top-10 72% vs 65% — better only on correlation (+0.34 vs +0.46). The rule was written into the driver before the run: an advantage that reverses where nothing was fitted is NQ curve fitting. **Two findings that outlive it:** (1) the HOLD CAP itself does not travel — on ES the uncapped R2 beats both capped variants (PF 1.360, MAR 0.43) and is the only one of the three positive in 2022, so an ES leg must not inherit the cap without its own evidence; (2) the tail bar FAILS on ES for all three (top-10 65-72%, both capped configs negative in 2022) — this family is materially more tail-dependent on ES than on NQ, a caution for the NQ+ES book line. **Also a correction:** the price-neutral test (every trade as a percent of its own entry price; mean entry 3,870 first half vs 14,408 second) shows the ~10x dollar gap between halves is 2.0-2.6x on per-trade expectancy and only 1.18-1.24x on profit factor — so `r3_leader_stress.py`'s "about twice as good" holds for expectancy but overstates profit factor, where it is about a quarter. No three-year block is unprofitable for any config (weakest 2013-2015, PF% 1.16-1.30); what changed after 2018 is the win rate (leader 29.8% → 35.3%).
- **2026-09-09 (buying back what the hold cap cost — `tools/r3_buyback_sweep.py`, `tools/r3_leader_stress.py`).** 54 cells around the shipped R3, moving the three knobs that trade the same thing the cap does (hold cap, trail, breakeven), each judged on the owner's tail bar plus MAR and held-out sample, all pre-registered. **20 of 54 clear every bar** (a plateau, not a corner) and the grid reproduces shipped R3 to the dollar. **LEADER = cap 9,660 / trail 2.0 / breakeven 1.5: n 2,992, net $588,793, PF 1.509, MAR 1.04, top-10 39.9%, corr +0.33, 2018 +$19,275, 2022 +$30,768, 172 held-out trades, longest hold 15 days** — beats shipped R3 (MAR 0.87 / 43.8% / +0.38 / 155) on every read; `queue_guard --tail-enforce` confirms PASS independently with reload and continuous held-out counts both 172. Cost stress 0.533→1.033: MAR 1.04→0.97 (−7%), still ahead of R3 and R2 at every cost, trade count flat. **PERIOD SPLIT is the caveat and it hits all three configs**: cut at 2018-06-30 the leader reads PF 1.252 / MAR 0.74 first half vs 1.568 / 1.89 second (R3 1.278/0.81 vs 1.560/1.57; R2 1.393/0.81 vs 1.788/1.65). The ~10x dollar gap between halves is mostly the index level (2,000 vs 20,000), but PF and MAR are scale-free and still say the edge was about twice as good after 2018 — a regime caveat for the card, NOT the beta problem again (the second half contains 2022). Also documented: act_R is inert when breakeven_R is 1.5, and breakeven 2.0 vs 2.5 is invisible once the trail has lifted the stop — identical grid rows are that degeneracy, verified against the exit block. Nothing crowned; the running full-discovery validate searches this same fenced space and decides.
- **2026-09-09 (the concentration rule, wired into the QUEUE GATE — `tools/queue_guard.py --tail` / `--tail-enforce`).** The tail / beta reads are now available where jobs are actually stopped, not just in the standalone checker. `--tail` prints them (shared implementation, `concentration_check.beta_block`, imported lazily because that module imports `resolve_params` from the guard); `--tail-enforce` makes them a bar — top-10 share < 60%, non-negative in EVERY benchmark down year, yearly correlation with the instrument < 0.40 — and a breach returns the new verdict **BETA** with **exit code 1**, so any queue script that already checks the exit code refuses to fire the job with no further change. Threaded through `--book-run` too (a BETA leg makes the book's worst exit 1). **OPT-IN on purpose**, for the same reason concentration is only ever a warning here: the program's own deployed legs fail it. Verified on the full window (2010-06-07..2026-06-30, split 2025-06-30): **R3 (crown + 8,280-bar hold cap) PASS** — top-10 44%, corr +0.38, down years +$20,491, 17/17 positive years, longest hold 13 days; **R2 (the live crown) → BETA** on correlation alone (+0.51), top-10 51%, down years +$19,732, longest hold 142 days. The default path is unchanged and re-verified: R2 with no flag still prints VERDICT PASS and exits 0.
- **2026-09-09 (the concentration rule, made enforceable — `tools/concentration_check.py`).** The owner's challenge ("those [metrics] are only bc of the fat tail … it was only working bc [the index] was going up … would need to make sure that concentration rule gets adhered to") is now a check anyone can run, not a one-off audit. Every run of the pre-queue gate additionally prints, off the SAME backtest: yearly net beside the instrument's own yearly return, the correlation of those two series, the benchmark's DOWN years called out separately, positive years with and without each year's three biggest trades, and the longest single hold. **Reported always; enforced only with `--enforce`** — top-10 share < 60%, non-negative in every benchmark down year, yearly correlation < 0.40 — because this program's own deployed legs are fat-tailed and working, so a default bar would fail them (same reasoning as queue_guard.py's header). On its window (2010-06-07..2025-06-29): **#335's search champion top-10 82% / corr +0.77 / down years −$54,250 FAIL; #309 53% / +0.59 / +$8,905 FAIL; R2 (the live crown) 51% / +0.51 / +$19,732 FAIL on correlation alone; R3 (crown + 8,280-bar hold cap) 44% / +0.39 / +$20,491 PASS**, longest hold 142 days → 13. **Also fixes THE {} TRAP in this file** — it passed the caller's params straight through, so `{}` ran the plugin's inherited signature anchor instead of its DEFAULT_PARAMS (on R3 that meant grading 2,655 trades / $324,648 / top-10 79% and condemning a configuration it never ran); it now resolves through `queue_guard.resolve_params()` and prints the resolved dict above the verdict.
- **2026-09-08 (BOOK #339 ENGU-Q NQ+ES; STUDIES rows 1416-1421, web v73.613).** Pre-LB book n/DD 14.9 vs NQ alone 13.3 (+12%), PF 1.53, 8/8 slices = bar cleared; LB +$137,136 / PF 1.70 / DD $69,144 vs NQ alone +$87,790 / DD $41,889; **ES leg alone LB +$17,496 / PF 1.27 / DD $33,074** (positive, drawdown-heavy). Candidate; single-leg ES validate = formal grade.
- **2026-09-08 (round 39 — build on the ES branch; STUDIES rows 1409-1415, web v73.611).** NOISE_1_0 searched on ES 5m RTH: 41/400 pass, leader PF 1.285 / MAR 10.8 / 7-of-8 / top-10 35% / plateau 26/28 → full-space ES validate queued (job 4Xic3kiQspP2CMiUdhtG, #243 window). ENGU-Q ES plateau above defaults (trail 3.0 PF 1.60 / n/DD 14.8) → NQ+ES BOOK queued (VgyMFCyNxjPvfTGcFVOt). GAPGO regime gates: none explain the LB loss, lead closed. `tools/queue_noise_es_validate.py`, `tools/queue_enguq_nqes_book.py`, `tools/r39_gapgo_regime.py`.
- **2026-09-08 (round 38 — run #334 PASSES, the cost question, the band is the trigger; STUDIES rows 1400-1408, web v73.610).** **Run #334 = `NOISE_1_0.py` full space on NQ 2m RTH (2010-06-07..2026-07-16, 8 folds, 17-mo LB): PASS 6/6, WF 7/8 (wfe 2.65), PBO 0.46, 7,885 trades, Sharpe 1.26; LB 453 tr / +$59,451 / PF 1.28 / DD $19,204 — beats #243 (+$40,648 / 1.19) and #305 (+$24,553 / 1.12) on the same market days (spent LB, confirmatory).** Champion (bands 0.5/1.0, VWAP exit, fixed stop 3.0, afternoon block) on the common window: n 7,427 / $335,478 / PF 1.315 / MAR 12.2 / R/YR 104 / hold 8 min; cost-fragile (MAR 10.7 at 0.783). Cost: 0.533 pts = $10.66 = ~2 ticks + commissions; the r37 fixed-target cells fail at zero cost too (best PF 1.11). All 74 gate-passing 2m cells re-scored at 0.533/0.783/1.033 (`tools/r38_noise2m_cost_stress.py`): 21 / 16 survive; the R/YR record (row 1321) drops MAR 19.6 → 11.4 → 5.4, the crown-#243 geometry holds 24.8 → 19.8 → 11.0, and the cost-robust corner (row 1403: band exit, ATR stop 0.75, confirm 3, afternoon block) holds 18.5 → 16.2 → 14.2 with 7/8 at every cost, $64/trade. `tools/r38_scalp2m_triage.py`: round-37's four triggers on 2m under NOISE's VWAP exit = 0/28 — the band is the trigger. 1m validate (job RboN8x79sMiYCBgbOySp) still running. Full doc: `NOISE.md` round 38.
- **2026-09-08 — cloud provisioning bundle (`deploy/cloud/`).** New: `README.md` (owner runbook — create a free Oracle Cloud Always-Free Ampere A1 VM, ssh in, one-command install, copy secrets from the PC, start/check/update/stop, what-runs-where table), `install.sh` (idempotent Ubuntu 24.04 ARM installer: python3.12 venv, `deploy/cloud/requirements-cloud.txt`, systemd units, safe to re-run), `edgelog-runner.service` (Restart=always) + `edgelog-healthcheck.service`/`.timer` (every 5 min, restarts the runner and pages ntfy on a 10-min-stale log), `check.sh` (status/log/disk/mem/secrets probe). Runs `api.runner` 24/7 with no NinjaTrader and no Windows PC in the loop — NT/live futures execution stays on the PC. Portability inventory only, not fixed: 26 hardcoded `C:\EdgeLog\...` path lines across 11 files in `api/` (9 already read an env override first, listed in the README table); `augur_engine/*.py` has zero hardcodes — `paths.py` is already Linux-portable (repo-root-relative via `__file__`), which is why the job-queue runner itself works on the cloud box today even though several Windows-PC side duties (NT-bridge readers, the QQQ shadow adapter's 10s feed) don't yet. No web VERSION bump — docs+scripts only, `index.html` untouched.
- **2026-09-08 (73.604) — NinjaTrader follows the ENGU-Q crown.** RegimeLen dll deployed (NT stopped, dll swapped, unattended recovery), `EdgeLogENGUQ1m` set to the R2 knobs through the bridge (check ok, Realtime). Parity of the port's never-before-live limit-entry / ER / regime paths = the nightly reconcile against `ENGUQ_335` from 09-09. Also: keep-aware fleet restart, `queue_guard --book-run`, runner ASCII startup line, Webull SDK log rotation silenced.
- **2026-09-08 (round 38 — scalpers part C; STUDIES rows 1375-1381, web v73.603).** Sweep-reclaim, chop-fade, volume ignition, 5-bar box break on NQ 1m RTH with FIXED (target x R, time stop) and RIDE (BE 1R, to close) exits, at 0.533 and 0.783/RT: **0/20**, every cell negative at house cost (fixed cells ≈ flat gross on 11k-43k trades = the round trip is the result; ride cells PF 0.87-0.95). Reversal/reversion dead at 1m too. Week: 0/59 short-hold cells across two sessions; NOISE 2m = the only paying short side. `tools/r38_scalpers.py`.
- **2026-09-08 (round 37d — ENGU-Q ES leg correlation).** ES vs NQ ENGU-Q R2 legs: daily corr 0.20 / monthly 0.21 / 685 shared entry days; pooled 1:1 n/DD 16.1 vs 13.2 (NQ) and 7.5 (ES, daily-aggregated) → the ES branch is a return diversifier for the ENGU-Q family. `tools/r37d_es_nq_corr.py`. ES validate GIrlgbDJ1umq12NrnfY8 in its walk-forward stage.
- **2026-09-08 (round 37 — ES branch triage; STUDIES rows 1367-1374, web v73.599).** The three crowned NQ files run unchanged on ES (`tools/r37_es_branch.py`): ORB dead (PF 1.06, 174% top-10), NOISE near miss (PF 1.236, 5/8), **ENGU-Q R2 crown clears the bar on ES 1m ETH at NQ defaults: n=2013 / $208,644 / PF 1.367 / DD $19,396 / n/DD 10.76 / 7-of-8; top-10 69%; survives 0.60 pts/RT; plateau 27/27**. Validate queued (job GIrlgbDJ1umq12NrnfY8, window pinned to #335, LB 12m). ES-vs-NQ leg correlation not yet measured.
- **2026-09-08 (73.594) — ENGU-Q CROWN → run #335 (R2).** The R2 sibling of #309 (breakeven 2.0 R, stop 1.0 ATR, logic untouched) validated PASS (6/6, WF 8/8, WFE 1.41, DSR 0.997, plateau 24/24, PBO 0.385, lockbox held) and the owner's standing instruction fired: paper leg `ENGUQ_335` on both boards, `ENGUQ_309` = matched control. Honest note recorded in `ENGUQ.md`: the validate's own best cell is a different configuration with a weaker held-out year, so the leg trades the R2 defaults the owner compared. NinjaTrader unchanged.
- **2026-09-08 (round 37 — the SCALP hunt; STUDIES rows 1315-1336, web v73.589).** Owner: beat the crowns from the SHORTER side. **Part B, five never-traded short-hold mechanisms on NQ 1m, 39 pre-registered cells (micro opening range, open drive, prior-day high/low break, range burst, VWAP cross; each with 1R / 2R / ride exits): 0 pass.** Every fixed-target cell loses or nets nothing (a 1R target on a 1m range cannot pay 2x cost); only ride-to-close cells earn (best MAR 5.1, holds 100+ min, top-10 71-376%) — not scalps. **Part A, the NOISE band on shorter bars: the bar size is the lever.** #243's config untouched on 2m = n 6,431 / PF 1.338 / DD $12,873 / MAR 24.8 / R/YR 96.9 / median hold 12 min (5m: 18.2 / 72.9 / 40 min); on 1m R/YR 113 but 5/8 slices and $29/trade. `tools/ryr_search.py` on `NOISE_1_0.py`'s own ranges, 2m (300 cfgs, 74 pass): **leader n=5,340 / $213,549 / PF 1.477 / DD $10,917 / MAR 19.6 / EV R 0.40 / R/YR 140.6 (library record; prior 102.6) / 6-of-8 / median hold 4 MIN / $40 a trade**; plateau 30/30 PF>=1.25, 27/30 keep half its R/YR; top-10 34%, ex-top-10 +$141,865 PF 1.32; queue guard PASS; daytype-OFF neighbour better (R/YR 162, MAR 23.8, 7/8). 1m (200 cfgs, 38 pass) ceiling R/YR 86 — not queued. **Queued: NOISE_1_0.py FULL space on the NQ 2m master, job `qsSDwagxJsahDSmAwx1r`** (2010-06-07..2026-07-16, 8 folds, 17-mo lockbox, spent-lockbox caveat on the card). Harnesses `tools/r37_scalp_triage.py`, `tools/queue_noise_shortbar_validates.py`; results `tools/r37_results/`. Full doc: `NOISE.md` round 37.
- **2026-09-08 (GAPGO family validates #330/#329 — FAIL on the lockbox; STUDIES rows 1310-1313, web v73.583).** Both seeds pass 6/6 gates on the selection window (#330 WF 8/8 wfe 1.77 PBO 0.27; #329 WF 7/8 wfe 2.39 PBO 0.44; plateaus HIGH GROUND; guard PASS, top-10 37%/44%) and both lose the sealed year 2025-08-13..2026-08-13: −$39,179 / PF 0.77 and −$15,923 / PF 0.90. June–July 2026 = median gap ~0.5 ATR vs 0.28 norm, every gap bucket lost, Databento-clean data; ORB crown +$88,943 in the same year → the gap/travel READS broke, not the opening-momentum factor. Family NOT validated, not handed over. `tools/gapgo_lockbox_year.py`.
- **2026-09-08 (rounds 35-36 — GAPGO becomes a FAMILY; STUDIES rows 1300-1309, web v73.579).** STREAK and FAILED FILL dead (17 cells). Hand-off controls: the gap direction is NOT the prior-day candle (PF 1.10, 135% top-10); ES transfer = same sign, quarter of the edge (PF 1.20, 72% top-10). **GAPGO TRAVEL 1.0** (`augur_strategies/GAPGO_TRAVEL_1_0.py`: at 10:00 |close−open| ≥ 0.3 ATR20 → continuation, stop at open) clears triage (n=1050 / $146,387 / PF 1.345 / DD $18,115 / n/DD 8.08 / 6-of-8; top-10 54%) and is the family's second trigger: 26% shared days with GAPGO, corr 0.24, +$157k on non-GAPGO days, pooled n/DD 9.79 vs 9.06/8.08. Overnight-range position = third read, 88% top-10, not adopted. Both validates queued (8 folds, LB 12m, ORB-crown window). Harnesses `tools/r35_family_seeds4.py`, `r36_family_seeds5.py`, `r36b_travel_vs_gapgo.py`.
- **2026-09-08 (rounds 32-34 — the NEW-FAMILY hunt; STUDIES rows 1288-1299, web v73.577).** Seven never-traded mechanisms pre-registered with `tools/concentration_check.py` logic in the harness: RELEASE (pre-event range break), VALUE AREA, WEEKLY-OR, VOLBAR, OVNGO/GAPGO, LEAD-LAG (ES→NQ), RETEST. **Six dead, 55 cells** (WEEKLY-OR is the instructive one: PF 1.21 on 1,093 trades but top-10 share 125%). **GAPGO 1.0 clears the house bar on triage** (`augur_strategies/GAPGO_1_0.py`: n=2007 / $151,024 / PF 1.341 / DD $16,672 / n/DD 9.06 / 8-of-8; parity exact; controls prove the gap DIRECTION is the edge — plain 5m ORB both sides PF 1.10). Marks: top-10 50% (ex-top-10 +$74,781 / PF 1.17); 63% of trade days are ORB-crown days, daily corr 0.19, ORB+GAPGO 1:1 n/DD 10.03 vs 10.32 alone → same opening-momentum factor, new trigger. Validate queued (8 folds, LB 12m, ORB-crown window). New family rail GAPGO n16. Harnesses `tools/r33_family_seeds2.py`, `tools/r33b_ovngo_controls.py`, `tools/r34_family_seeds3.py`, `tools/gapgo_vs_orb_overlap.py`.
- **2026-09-08 (v73.575) — runner read-meter counts the Firestore one-read-minimum on empty queries; commands-thread cleanup.** The `[reads]` meter added in v73.561 (memory `edgelog-firestore-quota`) was undercounting: an empty-result Firestore query still bills one read, and the meter had not been charging that floor, so an idle runner's true cost was invisible. Worker runners (`_run_worker.vbs 2..5`) now run no commands thread at all (they only drain jobs, per the 2026-09-07 concurrency change above); the primary's commands thread is a listener with a 600 s backstop poll rather than a tight interval loop. Net effect: the previously-invisible idle floor of ~29,000 reads/day is now backstop-only, not a per-tick charge.
- **2026-09-08 (v73.574) — `tools/queue_guard.py`: param resolution, resolved-params printout, like-for-like count check on `--run`.** Fixes the `{}`-params trap found while measuring the ENGU-Q R2 sibling against the #309 crown (see `ENGUQ.md` §3 2026-09-08 and `BOOKMARKS.md` B25): before this fix, `--params {}` on the ETH forks silently fell back to hardcoded module defaults instead of each file's own `DEFAULT_PARAMS`, and on 2026-09-08 it graded R2 on the #226 parity anchor (2,838 trades / $432,954) before the bug was caught. The guard now (1) resolves every omitted param from the target strategy file's own `DEFAULT_PARAMS`, (2) prints the fully-resolved param dict it actually ran so a silent substitution can't happen unnoticed again, and (3) on `--run N` cross-checks its own trade count against a like-for-like slice of that run doc before reporting a verdict.
- **2026-09-08 — throughput + queue integrity (v73.53x-57x).** The runner now runs FIVE jobs at once (primary + 4 drain-only workers; one launcher `C:\EdgeLog\_restart_runner.bat` starts the whole set at logon and on every restart, so a restart can no longer drop to one slot). Claimed jobs carry a 60 s heartbeat and the claimant pid; a sweep at boot and every 2 min requeues dead claims (running AND paused) and clears the dead runner's control flag. Boot chores (run-history/meta publish, master refresh - 59 s + 67 s warm) run on a thread so the first claim is seconds after boot; master writes are atomic. Run numbers are never invented: a counter outage saves the run PROVISIONAL and the primary renumbers it (the 1788836275 epoch id of 09-07 was the trigger). **Engine:** walk-forward folds run in parallel processes (`augur_engine/wf_pool.py`, `EDGELOG_VALIDATE_WORKERS`, default 3 in the launcher) - bit-identical rows to the in-line loop by construction (both call `auto._wf_fold_row`; pinned by `tests/test_wf_parallel.py`); Stage A stays sequential (adaptive search). **Pre-registered:** `NOISE_1_1_SBS_V90_SQT.py` = the compression finding as a fixed 2x SIZE TILT (same 27 fenced cells as #321; second contract pays its own round trip; decision rule in the docstring); `tools/queue_sq_tilt.py` queues it like-for-like with #321. Diagnostics: `tools/queue_truth.py` (queue by status vs the old newest-12 window).
- **2026-09-08 (RETRACTION — the EV R record was ten trades).** The round-28 ENGU-Q ER frontier config (EV R 1.40, plateau 26/26) **PASSED its Auto-Validate 6/6 (run #320)** and is still WITHDRAWN: **top-10 share of net = 101%**, ex-top-10 n=823 net=-$3,821 PF=0.977 EV R=-0.021. The round-29 frontier PAIR book is withdrawn with it (65% top-10; ex-top-10 EV R 0.20 not 0.57); its two book cards were already cancelled and are NOT requeued. **NOISE_1_2_RYR SURVIVES** (top-10 43%, ex-top-10 $84,029 / PF 1.187 / EV R 0.160) but its validate was WEAK (run #319, PBO only) - candidate only. NEW PRE-QUEUE GATE `tools/concentration_check.py` (>=90% top-10 or negative ex-top-10 = ARTIFACT). Lesson: plateau, walk-forward and concentration are THREE different questions - #320 passed the first two. STUDIES rows 1283-1286; web v73.570.
- **2026-09-07 — ENGU-Q `regime_len` unit FROZEN, documented on all three ETH forks (docs only, no executable line touched).** Verified by grep on origin/main: the 2026-08-26 ETH regime rescale (commit 6da54db, `rb = regime_len * ETH_BARS_PER_DAY`, §7 open items 2026-08-08/2026-09-05 above) was applied ONLY to `ENGUQ_1M_ETH_1_0.py`. `ENGUQ_1M_ETH_LIM_1_0.py`, `ENGUQ_1M_ETH_ER_1_0.py` and `ENGUQ_1M_ETH_ERW_1_0.py` still compute `rb = int(regime_len) * 390`, so `regime_len` on those three counts 390-bar blocks (~0.36 of an ETH day), not days — run #309's crowned `regime_len 10` is ~3.6 days, not 10. Measured the neighbourhood on #309's params (continuous run, entry-sliced, 2010-06-07..2026-06-30, split 2025-06-30, cost 0.533, mult 20): the crown's edge lives at ~3.6 days and a GENUINE 10-day filter (28 blocks at 1,091/day) is worse than no filter at all (selection PF 1.224 / net $203,744 / EV R 0.152 vs the crown's 1.661 / $505,756 / 0.439) — full table in `ENGUQ.md` under the crown section. Rescaling these three files would silently turn the crown into that worse row and break #309/#310/the queued ERW validate's own-file reproducibility, same failure mode as the #198 case above; the NT port `EdgeLogENGUQ1m.RegimeLen` (v73.559, `tools/nt/EdgeLogENGUQ1m.cs`) already mirrors the 390 math for the same reason. Added a prominent frozen-unit notice + the neighbourhood table to each file's module docstring and corrected the `regime_len` DEFAULT_PARAMS label/tooltip from "days" to "390-bar blocks" on all three — nothing else in any file changed. Verified: `ast.parse` clean on all three, and #309's exact params re-run through `augur_engine.engine.run_backtest` from the edited file still give n=1,604 / PF 1.655 / net $591,267, matching the run doc to the dollar.
- **2026-09-07 — Queue truth + runner concurrency + orphan sweep that works.** (1) v73.532 web: the top-bar QUEUE chip and BUILDER read the newest 12 job docs BY createdAt while the runner claims the OLDEST queued job first, so a requeued orphan that was RUNNING (and a script-queued job with no createdAt) were invisible - "5 WAITING, nothing running" with 1 running + 8 waiting in Firestore. Now one status-filtered live listener plus a one-shot recent read; `_liveJobs()` feeds both the chip and the new sticky BUILD QUEUE sidebar (RUNNING / WAITING in claim order / RECENT); `tools/queue_truth.py` (v73.589) prints the queue by status next to the old window. (2) `api/runner.py`: every running job stamps `heartbeat_at` every 60 s from its own thread and `claimedBy` names the runner pid; `sweep_orphans()` (boot + every 2 min on the primary, 10 min on workers) requeues a job whose heartbeat stopped for 4 min or whose claiming process is gone (`orphan_verdict()`, pinned by `tests/test_runner_orphans.py`). Replaces the boot-only 60-minute rule that missed the two jobs killed by the 21:03 / 21:33 restarts on 2026-09-06 (70% and 44% done, stranded for hours). (3) Worker runners (`C:\EdgeLog\_run_worker.vbs N`, EDGELOG_WORKER=N) now truly drain-only: NT bridge watchdog, QQQ shadow exec, data-health, fill review, drawdown alert, 9am preflight, nightly NT backup and auto-pine stay on the PRIMARY (before this only paper EOD and QQQ paper were guarded, so a worker would have double-pinged fills). Each runner refuses to claim while free RAM is under 2.5 GiB. Progress is written only when the whole percent changes and the STOP/PAUSE poll runs every 3 s (was a same-value write every 1.5 s). Measured before: a validate uses 0.97 of 14 cores, the box at 15-25 % busy; 1-minute ENGU-Q validates run 4-12 h each on one core. Modal.com assessed and declined: ~$5/month for the same wall-clock as local workers, plus parity and data-sync upkeep.
- **2026-09-05 (round 30 — family R / YR ceilings, like-for-like).** Same objective search on every family's own file/ranges: **NOISE ceiling R / YR 103** (368 tr/yr) = the frequency family; **ENGU-Q ER ceiling EV R 1.40** (55 tr/yr, R / YR 77) = the edge family and the library record; ENGU-Q ETH BASE ceiling EV R 0.45 / R / YR 82 (so ER is a better FAMILY, not a lucky config); **ORB ceiling R / YR 21** over 400 configs (EV R 0.13-0.17 - the floor, and why it dilutes books); TTM ceiling 12 (confirms closed). The frequency and edge ceilings live in DIFFERENT families - the reason the B19 pooled pair wins. STUDIES rows 1253-1257; web v73.493.
- **2026-09-05 (round 29 — frontier legs pooled).** `tools/book_ryr_frontier.py` pools every leg on ONE common pre-lockbox window with the real engine and scores as one strategy. **FRONTIER pair (NOISE_1_2_RYR + ENGUQ_1M_ETH_ER_RYR): n=6,381 / $403,949 / PF 1.656 / DD $17,900 / MAR 22.57 / EV R 0.566 / 424 tr-yr / R / YR 239.7 / 7-of-8 slices** versus the CROWNED pair of the same two families (NOISE #305 + ENGU-Q LIM #310) at PF 1.344 / DD $64,540 / MAR 9.77 / EV R 0.231 / R / YR 94.1 - it wins on EV R, R / YR, PF, DD and MAR at once; leg correlation +0.022. Frontier trio (+ORB #234) R / YR 189.4 / EV R 0.324 (ORB DILUTES - lowest EV R in the library at 0.150; a 400-config R / YR search over ORB_3_6 could not pass 21). All five legs R / YR 275.4 but EV R back to 0.278 and DD back to $75,417. Book cards queued UQI8tV4YPyZh7DqCCdSv (2-leg) + w60azTVlKDptxfCwfJRS (3-leg). CAVEAT on both: the frontier legs were selected on this window; their fenced validates (B88YSrSDg9KtupQXjJdI, Sl7Gxvj6cktTeQnOPRmZ) are the judges. STUDIES rows 1246-1252; web v73.487. BOOKMARKS B19-B20.
- **2026-09-05 (round 28 — R / YR objective search).** New tooling: `tools/ryr_search.py` (objective random search over a file's own declared ranges, scored on R / YR, engine-attached risk figures) + `tools/ryr_neighbourhood.py` (one-step plateau check). **TWO RECORDS FOUND, BOTH PLATEAUS, BOTH QUEUED AS FENCED 243-CELL NEIGHBOURHOOD VALIDATES:** (1) ENGU-Q 24h ER config at **EV R 1.40** (PF 2.553 / win 9.8% / n=833 / MAR 14.65 / R / YR 77.4 / 7-of-8 slices) - the highest EV R in the library vs 1.03 for the best prior validated single strategy; 26/26 neighbours hold; file `ENGUQ_1M_ETH_ER_RYR.py`, job B88YSrSDg9KtupQXjJdI. (2) NOISE config at **R / YR 102.6** (368 tr/yr, PF 1.325, EV R 0.279, MAR 8.96) vs 74-77 for the crowned geometry; 29/30 neighbours hold; entry window left OPEN because all_day scores 154.5 on only 5/8 slices; file `NOISE_1_2_RYR.py`, job Sl7Gxvj6cktTeQnOPRmZ. **NEGATIVE: NQDIP_1_1 validate FAILED (run #315, wfe 0.154, consistency fail)** - the 7-leg sweep's R / YR 23.5 / 8-of-8 did not generalise; #307 4-leg stands, 1.1 parked. STUDIES rows 1241-1245; web v73.484. BOOKMARKS B17-B18.
- **2026-09-04 (round 27 — EV R / R / YR hunt).** Owner: beat everything on EV R and R / YR, put sweeps on COMPARE, Auto-Validate the promising. (1) `tools/mine_ryr_frontier.py` reads every saved validate population: the single-strategy R / YR leader is an UNCROWNED #309 candidate (ENGUQ_1M_ETH_ER): pre n=2342 / PF 1.705 / win 14.9% -> EV R 0.60, R / YR 93 (crown 32; NOISE crown #305 77), WF 7/8, LB +$43.7k PF 1.29 -> own card queued (grid job 52SsjQYXXV6NBOdbPDDX). (2) NQDIP_1_1 (3 extra dip legs): R / YR 9.4 -> 23.5, PF 1.64, MAR 10.2, 8/8 slices -> validate queued 6FcGzxBn0LKbKNcMVlla. (3) EMAPB_1_0 1m EMA-pullback continuation DEAD 27/27. (4) Books: #311 (ORB+ENGUQ-ETH+NQDIP) PASS R / YR 82; #312 ENGU-Q NQ+ES pooled R / YR 48 (dilutes). NQDIP #307 PASS 6/6 ($388,627 / PF 2.02 / 654 trades), #308 QQQ FAIL. Family rules NQDIP/EMAPB added to the rail. STUDIES rows 1165-1177; web v73.479. BOOKMARKS B14-B16.
- **2026-09-02** — **Pre-push RUN-REPORT render gate.** `tools/preflight_boot.py` only proves index.html
  BOOTS; the RESULTS run report shipped broken through it three times in a week (v73.367 `_reXNm`
  undefined, v73.442 an `_hRow` without its heat getter, v73.443 the hotfix's own EV R row outside the
  row list), each blanking every run report on the live site until a hotfix. New
  `tools/report_render_probe.py` loads index.html headlessly the way the boot gate does, injects one real
  captured validate run (`tools/fixtures/run_report.json` = run #306, NOISE, gate_validate with 20
  candidates / 10 tilts / 5 hybrids; re-capture with `tools/capture_run_fixture.py <id>`), renders the
  report through `renderApp` exactly as a PAST RUNS click does under all three REPORT COLUMNS layouts,
  and FAILS on any `runDetail failed` console.error, any uncaught exception, or the "couldn't render"
  fallback card. Wired into `wt.py ship` beside the STUDIES / PAPER probes (runs when index.html changed;
  ~3 s). Verified: PASS on main, FAIL on the v73.442 and v73.443 builds. **Self-test added the same day:**
  `--selftest` pulls the v73.367, v73.442 and v73.443 builds from git history and asserts the gate still
  FAILS all three and PASSES the current build; `wt.py ship` runs it whenever the probe or its fixture
  changes, so the gate cannot go blind unnoticed.
- **2026-08-25 (round 26 final process)** — owner: Auto-Validate is the final step. Shipped `NQDIP_1_0.py` (the Nasdaq dip book as ONE ranged file: 4 long-only dip mechanisms side by side, constant-notional sizing + costs INSIDE the plugin, asset model auto-detected, house roll detector verbatim; equity curve built in exit order across legs). Registered a `QQQ 1d` Yahoo total-return master. Engine parity (fixed defaults, 2010-06-07..2025-06-29): NQ $333,424 / PF 1.74 / MAR 8.24; QQQ $335,922 / PF 1.72 / MAR 7.85. **Queued full-discovery Auto-Validates: NQ 5m job 8zkU44lPUNRtKR5IMWR2, QQQ 1d job n35b6uNdKpKRCt1CQ3nV** (cost_pts 0 / mult 1 by design, 8 WF folds, 12-mo lockbox). Next: BOOK job of the two champions once both land. BOOKMARKS.md updated.
- **2026-08-25 (round 26, WFO phase — THE NASDAQ BOOK).** Owner: oversample IS, judge by WF, any instrument/timeframe. Walk-forward optimization (per-fold whole-grid IS search -> OOS trade; OOS-only scoring) over 9 instruments x 4 dip mechanisms (8 folds), a new hourly intraday MR family (DEAD, pooled PF 1.00), and a dense 1,254-config / 12-fold Nasdaq grid. **Result: 8-leg Nasdaq book (QQQ + NQ micros, 4 mechanisms each) OOS-only n=1250 / $706,799 / PF 1.95 / DD $69,877 / MAR 10.11 / 12-of-14 yrs / post-2021 18% / bootstrap p=0 / corr 0.041 to champion; halves QQQ 8.55, NQ 9.18 (whole-MNQ 8.18). STACK on champion book 8.31 -> 11.20 (+35% vs +15% bar), net +85% => ADOPT BAR CLEARED.** Star leg QQQ 7-day-low buy 12/12 folds PF 2.54 MAR 12.2 (config drifts 7d/300 -> 15d/150 -> 6d/250). Caveats: recent half MAR 4.26; 1999-2010 holdout QQQ PF 1.26-1.35 / MAR 1.3-2.3 (bull-regime edge). Negative: broad 36-series re-optimized book MAR 3.05 < fixed r25 book 10.31. STUDIES rows 1141-1158; web v73.437. BOOKMARKS.md B11-B13.
...
- **2026-08-25 (rounds 20-23, autonomous)** — **0/40 more cells (hunt 0/144), map sharpened:** ORB mechanism decisively negative at Asia/London opens (the 9:30 open is the edge); no overnight segment holds anything (confirms the r18 retraction); turtle soup completes the every-fade-dies record; ES-ETH frozen ENGU-Q transfers at PF 1.125 (under bar) -> **full ES-ETH Auto-Validate with own param search QUEUED (job ktTjVhb2PRo9Ckg3rBFv, ES 1m eth, cost 0.613, 8 folds, 12mo lockbox)**; the crown risk engine is NOT portable (kills dips' multi-day edge, makes turtle worse than no-stop); ENGU-Q needs 1m granularity (5m port PF 1.11). STUDIES rows **1510-1549**; web v73.309. NOTE (updated 2026-09-09): this line read "rows 1027-1051; web v73.313" until then, because the v73.313 session that shipped rounds 24-25 overwrote this citation with its own block - 1027-1051 belongs to rounds 24-25, not to these. The rows themselves were written as 697-736 and became 1510-1549 in v73.647. They had collided with TTM Squeeze round 5, exactly as rows 592-616 (ORB crown travel + exit management round 2) collided with TTM rounds 2 and 4. All 65 duplicates were resolved by renumbering the later-discovered study of each pair, the TTM rounds having been discovered first; see STUDIES_BOARD.md section 9.
...
- **2026-09-07 (owner: "something instead of shots in the dark")** — **CROSS-FAMILY FEATURE BOARD + EXIT AUTOPSY shipped v73.538, LIFT (tail-aware read) v73.541** (COMPARE ▸ FEATURE BOARD). Verdict: nothing helps in three families at once; strategy-specific features only; exits are not the lever anywhere. Compression gate validate (`NOISE_1_1_SBS_V90_SQ.py`) is queued (job wyWdcdsZ), not yet run.

- **2026-09-07 — run #321 (NOISE-25) `NOISE_1_1_SBS_V90_SQ.py`, the 60m-compression GATE on the NOISE paper leg, fenced 27 cells: PASS 6/6**, WF 7/8 (rolling WFE 1.82), PBO 0.099 robust, DSR beats luck, lockbox strip 63 trades $38,383 PF 2.02. Full window (2010-06-07..2026-06-30) n=1,544 / $236,975 / PF 1.65 / DD $13,064 vs the UNGATED leg n=4,416 / $395,169 / PF 1.41 / DD $18,425 (LB $60,001 PF 1.36). Read: the coiled-state edge is REAL (per-trade quality up, lockbox PF 2.02) but as a FILTER it earns 40% less and its annualised MAR is LOWER (1.13 vs 1.34); the crown sat on the LOOSEST fence of all three knobs (30 min / len 16 / ratio 1.15, plateau: still climbing toward looser = admit more trades). Both facts point the same way as the round-6 writeup: use it as a SIZE TILT (2x when coiled), not a gate. Next = pre-registered tilt validate, owner call. Cross-check: FEATURE BOARD lift for `compressed_60m` is positive on 5/6 legs and in every lockbox but clears its bar only on NOISE #243.

- **2026-09-07 — NOISE momentum SIZE TILT validate (run **#327** NOISE-27, `NOISE_1_1_SBS_V90_MT.py`, 18 fenced cells, control = #243): WEAK 5/6** — WF 8/8 (WFE 2.77), DSR ok, but **PBO 0.60 (likely overfit selection)** and the crown 30 bars / 3.5 ATR / 2x sits on the mom_atr FENCE (min). Same-window comparison vs the untilted leg: pre-lockbox $551,662 / PF 1.494 / DD $24,561 (vs $320,130 / 1.42 / $18,425), full $646,458 / PF 1.441 / DD $34,808 (vs $380,745 / 1.387 / $22,096), 18-month lockbox 349 trades $68,159 / PF 1.205 / DD $34,808 (vs $40,648 / 1.191 / $22,096). MAR: full 1.15 vs 1.06, lockbox 1.31 vs 1.23. Read: a consistent but MODEST risk-adjusted gain in every window, the 18 cells are indistinguishable (PF 1.46-1.49, hence PBO), and the search wants an even lower threshold = tilt more trades = drifting toward plain leverage. Pre-registered adopt bar NOT met (PASS, PBO, interior crown all failed). NOT adopted; the anatomy lead is closed as 'real but too small to separate from leverage'. Note the pre-registered lockbox-PF bar (1.36) was mis-specified from the 12-month round-6 number; the control's own 18-month lockbox PF is 1.191.

- **2026-09-08 — ENGU-Q TRADE ANATOMY, 6 runs (#309 crown, #226, #326, #310, #320, #227 RTH; #198 excluded, its file changed since the run): `docs/anatomy/COMPARE_ENGUQ.md`.** All six reproduce their run docs to the dollar (1-minute replays now take ~1 min each after vectorising the trailing-year percentile and keying the daily table by trading day). SIX pre-entry conditions repeat with the same sign in >= 3 runs, significant in >= 2, and hold in every run's own holdout: expanding 5-minute volatility (Bollinger-width and ATR percentile HIGH) and a DIP context (price BELOW the 20/50/200-day averages, LOW daily RSI). That is ENGU-Q's structure: its big winners are reversals caught in high intraday volatility after a selloff. BUT every skip rule built on them is a REGIME ARTIFACT (rsi14_daily>p70 = artifact in 3 of 3 runs, sma50d>p70 in 2 of 2); the one #309 rule that 'carries' (skip when >14.7 ATR above the 60-min EMA50, +2%/+5%) carries on #226 too but is an artifact on #326 and 'no' on #227 -> NOT robust. 0 ROBUST rules, 0 tilts. Read: the structure ranks trades, it does not license skipping any; same conclusion as NOISE/ORB. Caveat: the six runs share most trades (variants of one strategy on one tape), so cross-run agreement is weak evidence; the within-run holdout is the real check.

- **2026-09-08 — 'MORE IDEAS' ROUND, three pre-registered lines, 2025-07+ never loaded.** (1) New information into trade_anatomy (calendar incl. FOMC±1/OPEX/month-end, NQ-vs-ES relative strength, overnight session, vol term structure; 29 columns; `docs/anatomy/COMPARE_NEWFEAT.md`): NOTHING new carries — all 8 ledger rules on new features are regime artifacts; only lead `xm_relret_60m_atr` on #326 alone. (2) Four new NQ mechanisms, scan only (`tools/leg_scan_ideas.py` -> `docs/anatomy/LEG_SCAN_IDEAS.md`, 40 cells): 0 LEADS (PF>=1.25 & MAR>=1 & n>=80 in both windows); closest = gap FADE >=0.3 ADR when the overnight did NOT already retrace (PF 1.43/1.46, 12-17 trades/yr, ~$29k/15y, no single-year artifact); confirmed both windows: intraday CONTINUATION beats reversion (NQ-vs-ES spread, last hour); FOMC 2pm breakout unusable (19 triggers/15y); OPEX fade and month-end long dead. (3) Weak-edge ETF BOOK made real: daily masters GLD/TLT/IWM/QQQ(+SPY) as 1d/rth/yahoo_adj (files in `C:\EdgeLog\_anatomy_cache\etf_masters\`), `ETFDIP_{DBL7,RSI2,PB20}_1_0.py` plugins with 16/16-leg parity to the dollar vs r25, `tools/queue_etf_book.py` (job built, NOT sent). Local run_book dry-run of that job: PASS 8/8 slices, n=1,608, net $805,277, PF 1.73, DD $73,194, lockbox 137 trades +$85,335 PF 1.67. CAVEATS: Yahoo total-return prices re-scale every dividend, so r25 no longer reproduces its own 2026-08-25 numbers (11 legs not 15; PF 1.66, raw MAR 6.4-8.1 vs 1.52/10.31) — frozen master CSVs fix this from here; the runner needs the 5 CSVs copied into the shared `augur_uploads/` + `csv_files` rows before `--send`; use `--warm` (2009-06-01) so the 200-day filter is warm. Queue = owner call.

- **2026-09-08 — run #332 (BOOK-13) = the ROUND-25 WEAK-EDGE BOOK through the house BOOK scorer (7 ETF dip legs GLD/TLT/IWM/QQQ + NQDIP_1_0, job ByLgEpDazSxhiITAy1qG, window 2010-04-28..2025-06-25, lockbox from 2024-06-25): PASS.** Whole n=1,676 / $832,313 / PF 1.72 / DD $73,194 / raw net-to-DD 11.4 (pre-registered gates PF>=1.25 and net/DD>=8 both cleared), 8 of 8 slices held, lockbox 137 trades +$85,335 PF 1.67. HONEST MARKS: (a) the lockbox year carried a $61,419 drawdown against its $85k gain (the 2025 spring selloff) and that is most of the book's whole-window max DD; annualised MAR is only 0.71 at $100k notional per ETF leg; (b) NQDIP_1_0 supplies 40% of the net ($333k of $832k, already validated as #307) — the ETF legs add $499k on their own and three of the seven are QQQ variants (correlated); (c) prices are Yahoo total-return, frozen as master files (csv_files ids 52-56) — the live series will not match the study tick-for-tick. Read: the cross-asset dip book is the first NEW leg family to pass the house since NQDIP; it is a STOCKS-account book. Next = ETF-only sub-book numbers (without NQDIP) and a paper leg via the QQQ shadow adapter, owner call.

- **2026-09-08 — run #338 (BOOK-16) = the ETF-ONLY sub-book (the 7 ETF legs of #332 without NQDIP, `queue_etf_book.py --etf-only --warm`, job 4rXduqU93419JFbiYqlL): PASS.** Whole n=1,047 / $498,889 / PF 1.71 / DD $43,818 / raw net-to-DD 11.4, 8 of 8 slices, lockbox 86 trades +$53,879 PF 1.69 (lockbox DD $35,549). Read against #332 ($832k / PF 1.72 / DD $73k / net-to-DD 11.4): the ETF legs carry the SAME profit factor and the same net-to-drawdown on their own — NQDIP adds dollars ($333k) and drawdown ($29k) in equal proportion, not quality. The ETF dip book stands alone as a stocks-account book. Shadow paper leg ETFBOOK_332 being built (no orders).

- **2026-09-08 — PAPER LEG `ETFBOOK_332`, the ETF dip book on the board as SHADOW ONLY (`api/etf_book_shadow.py`, leg registered in `api/paper.py` PAPER_LEGS, live_from 2026-09-09).** The seven ETF legs of #332 / #338 (GLD-DBL7, TLT-DBL7, IWM-RSI2-both, QQQ-DBL7, QQQ-RSI2, QQQ-RSI2-both, QQQ-PB20) pooled into ONE paper row at $100k notional a trade and $20 a round trip — the exact params `tools/queue_etf_book.py --etf-only` emits. It is the first paper leg that does not run on the NQ tape: once per trading day at/after **16:15 ET** the runner's watch loop calls `etf_book_shadow.maybe_nightly_update(q)` (immediately after `paper.maybe_run_eod`), which pulls the day's GLD/TLT/IWM/QQQ bar from Yahoo, APPENDS it to the frozen 1d masters (csv_files 52–56) without ever rewriting history, re-runs the seven legs, diffs the position set against yesterday's and writes this leg's rows through `paper.rerun_legs`. Two guards worth knowing: (a) a Yahoo/master mismatch on a date the master already holds is classified before anything is touched — the same ratio across the last six bars is a DIVIDEND RE-SCALE and history is left alone and logged, the last bar alone is an unfinished bar written mid-session and only that one row is replaced; (b) open positions are recovered by PROBING the plugin with synthetic high/low bars rather than transcribing the three rules a second time, checked against full-history ground truth in `tests/test_etf_book_shadow.py`. **NOTHING IS EVER SENT TO A BROKER** — there is no order path in the module and a test greps for one. Owner action to activate: restart the runner (`wscript C:\EdgeLog\_restart_runner_hidden.vbs`).

- **2026-09-09 — run #360 (NOISE-33) = the EVIDENCE RUN for the candidate board** (`NOISE_1_0.py`, job d45LrW1a, first use of `save_fold_detail=True` + `oos_sample_k=30`, 39b2f52). It answers what the board could not: the ten finalists ARE better than the field. Sealed year, 10 finalists vs 30 configs drawn across all ten in-sample deciles: mean net **+$6,224 vs -$930**, median **+$9,423 vs -$2,779**, positive **6/10 vs 10/30**, PF **1.072 vs 0.877**, per-trade **+$40.7 vs -$38.1**. Mann-Whitney one-sided: per-trade **p=0.039**, PF p=0.057, net p=0.127 (n=10 vs 30 in ONE run, so direction is consistent on all four measures but only the per-trade test clears 5%). Within the same run, walk-forward rank still fails to order the sealed year across all 40 configs (rho +0.16, p=0.32), matching the board's rho +0.02 over 46 runs. **Read: the SHORTLIST carries information, the TIE-BREAK does not.** Keep the search and the top-k; stop treating the crowned cell as special (carry finalists as a set / pool them). Next: leave `oos_sample_k=30` on for the next few validates (+~3% wall clock, +52KB) to turn one suggestive run into a real sample. Per-fold rows are now on every candidate too (`selection.candidates[i].folds`, 8 rows each, untrimmed).
