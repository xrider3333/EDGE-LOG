"""
NOISE 1.2 CHOP — NOISE_1_0 + a Kaufman efficiency-ratio CEILING on entries.

2026-09-04 -- fork of NOISE_1_0.py (byte-for-byte parent copy plus one new gate).
Concept: the mean-reversion crown (NOISE) fades band touches, betting the tape
snaps back. That bet should do BETTER when the recent tape is choppy (a band
touch is more likely to be noise) and WORSE when the tape is trending hard (a
band touch is more likely to be the start of a real move that keeps going --
exactly the case where fading it is most dangerous). The Kaufman efficiency
ratio (ER = |close[i]-close[i-L]| / sum(|bar-to-bar close changes| over the
same L bars)) is the one confirmation mechanism in this project that has ever
survived a pre-registered bar -- as a FLOOR (require ER >= threshold, i.e.
"only trade when the tape IS trending") on ENGU-Q's 1m ETH momentum entry
(ENGUQ_1M_ETH_ER_1_0.py, run #265). This file uses the SAME ratio the OTHER
way: as a CEILING (er_max) -- skip a NOISE entry when the recent tape's ER
EXCEEDS er_max, i.e. skip entries when the tape has been moving too
efficiently (too trend-like, not choppy enough) over the last er_len bars.
ER is computed on CLOSES ONLY, globally across the master's bar index (same
convention as ENGUQ_1M_ETH_ER_1_0.py's er_th gate), using data up to and
including the signal bar -- fully causal. New knobs: er_max (float, default
0.0 = OFF/parity, range 0.10-0.60 step 0.05) and er_len (int, default 12 bars
of 5m = one trading hour, range 6-48). Both default to a byte-identical
no-op -- every reference number in the ORIGINAL NOISE_1_0.py docstring below
still applies unchanged when er_max=0.0. See tools/noise_chop_bench.py for the
pre-registered bars, parity checks and bench grid.

──────────────────────────────────────────────────────────────────────────────
Everything below this line is the UNMODIFIED NOISE_1_0.py docstring, kept for
context (mechanism, prior research, validation status of the PARENT file --
none of it has been re-earned by the er_max gate above, which is brand new and
unvalidated until tools/noise_chop_bench.py's bars clear).
──────────────────────────────────────────────────────────────────────────────
...
STRATEGY_NAME = 'NOISE 1.2 CHOP · intraday momentum envelope + efficiency-ratio ceiling'
DESCRIPTION   = ("NOISE_1_0's wide-band VWAP-exit envelope, plus an optional CEILING on "
                 "the Kaufman efficiency ratio of the last er_len closes: skip an entry "
                 "when the recent tape has been moving too efficiently (too trend-like) "
                 "to trust a mean-reversion fade. Default er_max=0.0 = OFF, byte-identical "
                 "to NOISE_1_0. UNVALIDATED new knob -- see tools/noise_chop_bench.py.")
...
DEFAULT_PARAMS = {
    ...
    "er_max": {
        "default": 0.0, "min": 0.10, "max": 0.60, "step": 0.05, "type": "float",
        "label": "Efficiency-ratio ceiling (0=off)",
        "tooltip": "0.0 (default) = no change, byte-identical to NOISE_1_0. Otherwise: "
                   "skip a NEW entry whenever the Kaufman efficiency ratio of the last "
                   "er_len closes (net move / total path length over that window, closes "
                   "only, up to and including the signal bar -- fully causal) EXCEEDS "
                   "this value. ER near 1.0 = the tape has moved almost in a straight "
                   "line (trending, efficient); ER near 0 = a lot of back-and-forth for "
                   "little net progress (choppy). NOISE fades band touches betting on "
                   "reversion, so this ceiling tries to skip the touches most likely to "
                   "be the START of a real trend continuation rather than noise. UNVALIDATED "
                   "-- new 2026-09-04 knob, see tools/noise_chop_bench.py for the bench.",
    },
    "er_len": {
        "default": 12, "min": 6, "max": 48, "step": 1, "type": "int",
        "label": "Efficiency-ratio window (bars)",
        "tooltip": "How many trailing 5m closes feed the efficiency-ratio ceiling above. "
                   "12 = one trading hour (5m bars). Only read when er_max > 0. UNVALIDATED "
                   "-- see tools/noise_chop_bench.py.",
    },
}
...
def _efficiency_ratio(c, L):
    """Kaufman efficiency ratio, computed GLOBALLY over the full close array (same
    pattern as ENGUQ_1M_ETH_ER_1_0.py's er_th gate -- the one confirmation mechanism
    in this project that has survived a pre-registered bar, run #265). er[i] = the
    net move over the trailing L closes ending at i, divided by the total bar-to-bar
    path length over the same L bars: |c[i]-c[i-L]| / sum_{j=i-L+1..i} |c[j]-c[j-1]|.
    Uses closes up to and including index i only -- fully causal. NaN for the first
    L bars (insufficient history); 0.0 where the path length is exactly zero."""
    n = len(c)
    L = int(L)
    if L < 1 or n <= L:
        return np.full(n, np.nan, dtype=float)
    shifted = np.concatenate([np.full(L, np.nan), c[:-L]])
    chg = np.abs(c - shifted)
    ad = np.abs(np.diff(c, prepend=c[0]))
    cs = np.cumsum(ad)
    vsum = cs - np.concatenate([np.zeros(L), cs[:-L]])
    with np.errstate(invalid="ignore"):
...
def run_backtest(
    ...
    er_max: float = 0.0, er_len: int = 12,
    day_id=None,
    ...
    # er_max CHOP gate (2026-09-04) -- computed ONLY when the knob is on, so the
    # default (0.0) stays a true no-op (byte-identical to NOISE_1_0; proven by the
    # smoke tests below). Global bar-index array, same convention as vol_pct/dt_pos
    # being read per-bar via the absolute index a+k inside the session loop.
    er_full = _efficiency_ratio(c, er_len) if er_max > 0.0 else None
        ...
        for k in range(m):
            ...
            if pos == 0 and not is_last and 1 <= k <= m - 2 and not sess_block_entries:
                ...
                if window == "morning":
                    ...
                    in_window = (k <= m - 26)
                # er_max CHOP gate (2026-09-04): skip this bar's entry if the trailing
                # er_len-bar efficiency ratio (closes only, causal, up to and including
                # THIS bar's close) exceeds the ceiling -- the tape has been moving too
                # efficiently (too trend-like) to trust a mean-reversion fade here.
                # NaN (insufficient warmup) is NOT blocked (matches vol_pct/dt_pos
                # convention of "inactive filter" on NaN).
                if er_full is not None:
                    _e = er_full[a + k]
                    if not np.isnan(_e) and _e > er_max:
                        in_window = False
                if in_window:
                    ub_k, lb_k = UB[k], LB[k]
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — reruns NOISE_1_0.py's own (a)-(d) parity checks THROUGH THIS FORK
# (er_max=0.0 default on every one -> must reproduce the parent's exact certified
# numbers, proving the new gate is a true no-op when off).
#   Run:  python augur_strategies/NOISE_1_2_CHOP.py
# ─────────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    ...
    def _run(label, extra_params, expect_n, expect_net, expect_dd, expect_pf=None,
        ...
        r = eng_bt("NOISE_1_2_CHOP.py", instrument="NQ", timeframe="5m", session="rth",
