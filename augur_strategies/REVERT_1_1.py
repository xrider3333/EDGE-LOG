...
# -- Identity -----------------------------------------------------------------
STRATEGY_NAME = 'REVERT 1.1 · mean-reversion + regime filter'
...
DEFAULT_PARAMS = {
    ...
    "use_regime": {
        ...
        "label": "Regime filter (don't fade trends)",
        "tooltip": "Only fade when the market is RANGING (low Kaufman efficiency ratio); "
                   "stand aside when it's trending, so a trend can't run your fades over.",
    },
    "regime_lb": {
        "default": 30, "min": 10, "max": 80, "step": 5, "type": "int",
        "label": "Regime lookback", "depends_on": {"use_regime": True},
        "tooltip": "Bars over which the efficiency ratio is measured.",
    },
    "regime_max_er": {
        "default": 0.4, "min": 0.15, "max": 0.7, "step": 0.05, "type": "float",
        "label": "Max efficiency ratio to fade", "depends_on": {"use_regime": True},
        "tooltip": "Fade only when ER is below this (more ranging). Lower = stricter.",
    },
}
...
def run_backtest(
    ...
    use_regime: bool = False, regime_lb: int = 30, regime_max_er: float = 0.4,
    ...
    # Efficiency Ratio (Kaufman) for the regime gate: |net move| / |total path| over
    # regime_lb bars. ~1 = clean trend, ~0 = chop. We fade only when it's LOW (ranging).
    er = None
    if use_regime:
        _absd = np.abs(np.diff(closes, prepend=closes[0]))
        er = np.full(n, np.nan); _rl = max(2, int(regime_lb))
        for _i in range(_rl, n):
            _net = abs(closes[_i] - closes[_i - _rl])
            _path = float(_absd[_i - _rl + 1:_i + 1].sum())
            er[_i] = (_net / _path) if _path > 1e-9 else 0.0
    ...
    for i in range(start_idx, n):
        ...
        if flat_eod and _send is not None and _send[i]:
            continue
        # Regime gate: stand aside when the market is trending (high ER), so a trend
        # can't run our fades over — the exact failure that sank fold 5 of REVERT 1.0.
        if use_regime and er is not None and not np.isnan(er[i]) and er[i] >= regime_max_er:
            continue
