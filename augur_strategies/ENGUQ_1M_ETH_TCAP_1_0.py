"""
ENGU-Q 1m ETH - TIME CAP (research -> candidate, campaign 2026-09-05)
--------------------------------------------------------------------------------
Fork of ENGUQ_1M_ETH_ER_1_0.py (the efficiency-gate engine) adding ONE new knob,
`max_hold_bars`: force-close a position at that bar's CLOSE once it has been open
this many bars, regardless of stop/trail state.

WHY: memory `edgelog-evr-tail-detector` / ENGUQ.md section 1.0 measured that on this
family's 24-hour tape, EV R >= 0.9 has ALWAYS meant top-10 share >= 90% -- a wide
trailing exit (trail_frac 4.0-5.0) with no end-of-day flat never triggers, so one
winner becomes a months-long hold and the "average trade" IS that hold (#310: EV R
0.94, top-10 share 90%, a 449-day hold, ZERO continuous held-out trades). A time cap
attacks the mechanism directly: force the months-long hold to realise, which should
cut top-10 share, raise trade count, and leave the genuine per-trade edge intact.

`max_hold_bars` = 0 is the PARITY ANCHOR: OFF, bit-identical to ENGUQ_1M_ETH_ER_1_0
(and, with er_th=0 too, to the certified #226 engine: n=2843 / $434,721.12).
At 1-minute ETH bars, one 24h session is ~1380 bars: 1380 = one day, 6900 = one
week, 20000 = about a month.

The cap fires INSIDE run_backtest's position-management loop, exactly like the
stop/trail/breakeven exits -- never as a post-hoc filter on the trade list, because
a forced exit here FREES the strategy to take a later signal it would otherwise have
missed while still holding.

PRE-REGISTERED BARS (written before running; all must pass for a cell to be PROMISING):
...
STRATEGY_NAME = "ENGU-Q 1m ETH · TIME CAP (research)"
...
DEFAULT_PARAMS = {
    'max_hold_bars': {'default': 0, 'min': 0, 'max': 20000, 'step': 100, 'type': 'int',
