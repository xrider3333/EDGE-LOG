...
`rb = regime_len * ETH_BARS_PER_DAY`) was applied ONLY to ENGUQ_1M_ETH_1_0.py; this file
(and its ER/ERW children, forked from it) were deliberately left on the old 390 math. Run
#310 (limit_atr 0.7 / trail_frac 4.0 / regime_len 5, THIS file) and run #309
(ENGUQ_1M_ETH_ER_1_0.py, regime_len 10 - forked from this file) both depend on that
convention: #309's regime_len 10 = 3,900 bars ~= 3.6 calendar days, not 10 days.

Measured on the #309 params (same regime code as this file), one continuous run sliced by
entry time (NQ 1m ETH 2010-06-07..2026-06-30, split 2025-06-30, cost 0.533, mult 20):
...
row) is WORSE than no filter at all. Rescaling this file to 1,091 would silently turn #309
into that worse row and change what #310's own regime_len=5 means too, and neither run
would reproduce from its own file any more - the run #198 lesson, BACKTESTING_STACK.md
2026-08-08/2026-09-05. The NinjaTrader port `EdgeLogENGUQ1m.RegimeLen` (v73.559,
tools/nt/EdgeLogENGUQ1m.cs) deliberately mirrors this same 390 math for the same reason.
DO NOT rescale `regime_len` on this file without re-validating both runs from scratch.
"""
import numpy as np
...
DEFAULT_PARAMS = {
    ...
    'regime_len': {'default': 0, 'min': 0, 'max': 100, 'step': 5, 'type': 'int',
                  ...
                  'tooltip': 'Only go long when close is above its N-BLOCK simple average '
                             ...
                             'FROZEN unit, do not rescale without re-validating. Run #310 '
                             '(this file) = 5 blocks ~= 1.8 days; run #309 (ER fork) = 10 '
                             'blocks ~= 3.6 days.) 0=off.'},
