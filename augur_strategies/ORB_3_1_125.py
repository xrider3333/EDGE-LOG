...
# ── DEPLOY-LOCK (appended): pin the search space to run #125's exact deployable config
#    (or1 / stop 0.75 / trail 5 / Both, all other levers off) so Auto-Validate discovers
#    ONLY this config and produces a report card comparable to runs 137 (3.0) / 150 (3.1
#    short-only). flat_eod / skip_holidays are intentionally dropped -> run_backtest
#    signature defaults (True / False) = 125's values. A later module-level assignment wins.
STRATEGY_NAME = "ORB 3.1 · 125 deploy-lock (or1/stop.75/trail5/Both)"
DEFAULT_PARAMS = {
    "or_bars":        {"type": "int",   "min": 1,    "max": 1,    "step": 1,    "default": 1,    "label": "Opening range (bars)"},
    "trade_mode":     {"type": "str",   "options": ["Both"],                    "default": "Both", "label": "Direction"},
    "stop_frac":      {"type": "float", "min": 0.75, "max": 0.75, "step": 0.25, "default": 0.75, "label": "Stop (x range)"},
    "vol_filter":     {"type": "float", "min": 1.25, "max": 1.25, "step": 0.25, "default": 1.25, "label": "Volume filter"},
    "breakout_buf":   {"type": "float", "min": 0.0,  "max": 0.0,  "step": 0.05, "default": 0.0,  "label": "Breakout buffer"},
    "partial_exit_R": {"type": "float", "min": 0.0,  "max": 0.0,  "step": 0.5,  "default": 0.0,  "label": "Partial exit / lot-1 TP"},
    "trail_bars":     {"type": "int",   "min": 5,    "max": 5,    "step": 1,    "default": 5,    "label": "Trailing stop (bars)"},
    "atr_filter":     {"type": "float", "min": 0.0,  "max": 0.0,  "step": 0.1,  "default": 0.0,  "label": "Vol-regime filter"},
    "target_R":       {"type": "float", "min": 0.0,  "max": 0.0,  "step": 0.5,  "default": 0.0,  "label": "Runner target"},
}
PARAM_GRID_PRESETS = {"125 deploy-lock": {"or_bars": [1], "stop_frac": [0.75], "trail_bars": [5]}}
