"""
ORB 3.6 — PINNED CANDIDATE G115: STUDIES row 115, the grid survivor never validated.

Regime filter OFF, buffer 0.30, stop 2.50 on the ride+BE exit. Cleared the same pre-registered robustness gate as runs 257/259/260 but never got a validation slot.

Offline (NQ 5m RTH no-adj, 0.533 pts/RT, 1 contract, run #230's window):
    full net $409,912  ·  maxDD $32,505  ·  lockbox $91,405
    (crown #234: $389,874 · $29,142 · $88,943)

Owner 2026-08-19: validate these two alongside an OPEN Auto-Validate of ORB_3_6.py
(the ML-features run). Judge on WF folds vs #234's 7/8 / wfe 4.649, never on the
offline net. The shared 2025-08-13..2026-08-13 lockbox is heavily reused — treat its
dollars as encouraging, not confirmatory. REGIME CAVEAT: money is post-2018.
...
_spec = _ilu.spec_from_file_location("_orb36_base_g115", _SRC)
_base = _ilu.module_from_spec(_spec)
_spec.loader.exec_module(_base)

STRATEGY_NAME = 'ORB 3.6 · candidate G115 — STUDIES row 115, the grid survivor never validated, pinned'
DESCRIPTION   = ("Regime filter OFF, buffer 0.30, stop 2.50 on the ride+BE exit. Cleared the same pre-registered robustness gate as runs 257/259/260 but never got a validation slot.")
