"""
OPENING RANGE BREAKOUT 3.8 - the crowned legal base (3.6 / run #234 "ride+BE") plus the
PRIOR-DAY CLOSE-POSITION filter found by the 2026-08-17 NOISE campaign.

WHY THIS FILE EXISTS (2026-08-18 cross-family transfer test): the NOISE campaign found
that skipping SHORT entries on the day after a session closed in the bottom 20% of its
own range was a large, broad, robust improvement (+$52,200 full window, 98.6% of it on
the short side, positive in 14 of 17 years). The same "shorts fail after weak closes"
shape had been noticed on 4+ strategy families in this program but had NEVER been built
and tested as an explicit filter on ORB - the crowned, live-in-NinjaTrader family. This
...
H/L/C are that PRIOR session's high, low and close. NaN on the first session or on a
zero-range session, and the filter is inactive whenever it is NaN.

LIVE-LEGAL BY CONSTRUCTION: every input is a FINISHED prior session's bar. Nothing on
today's tape, let alone the fill bar, is read. daytype_mode='off' reproduces ORB_3_6.py
bit-for-bit (asserted in the __main__ smoke test).

Knobs = 3.6's thirteen + daytype_mode / daytype_lo / daytype_hi.
"""
import numpy as np

STRATEGY_NAME = 'ORB 3.8 · legal base + breakeven + prior-day close-position filter'
DESCRIPTION   = ("The crowned legal ORB (close-confirm entries, v-pace/ATR-regime "
                 "gates, ride to target with breakeven) plus ONE new gate: where "
                 "YESTERDAY closed inside its own range. Skip shorts (or both sides) "
                 "after a bottom-20 percent close, or longs after a top-20 percent "
                 "close. Fully causal - prior finished session only. Default OFF = "
                 "ORB_3_6 exactly.")
...
DEFAULT_PARAMS = {
    ...
    "daytype_mode": {
        ...
        "tooltip": "Gate today's entries on where YESTERDAY closed inside its own range "
                   ...
                   "off (default) = no change, identical to ORB 3.6. skip_bot_short = "
    ...
    "daytype_lo": {
        ...
        "tooltip": "The skip_bot_* cutoff: yesterday's close-position at or below this "
                   "counts as a weak close. 0.20 is NOISE's researched value; 0.15 and "
                   "0.25 are the pre-declared plateau neighbours. Only read when "
                   "daytype_mode is a skip_bot mode.",
    },
    "daytype_hi": {
...
def _daytype_pos(h, l, c, sess_bounds):
    ...
    (C-L)/(H-L). NaN for the first session or a zero-range session (the filter
    is inactive there). Identical definition to NOISE_1_0.py's `_daytype_pos`
    so cross-family results are comparable. Causal - prior finished session."""
...
def run_backtest(
    ...
    # ── Prior-day close-position filter (2026-08-18) ─────────────────
    # Computed ONLY when the knob is on, so the default stays a true no-op.
    _dt_pos, _dt_ord = None, None
    if daytype_mode != "off":
        _dt_pos = _daytype_pos(h, l, c, _sess_bounds)
        _dt_ord = {a: si for si, (a, b) in enumerate(_sess_bounds)}
    ...
    while i < n:
        ...
        if m > or_bars + 1 and or_bars >= 1:
            ...
            if rng > 0:
                ...
                # PRIOR-DAY CLOSE-POSITION GATE (causal: prior finished session only)
                if _dt_pos is not None:
                    _dsi = _dt_ord.get(i)
                    _dp = _dt_pos[_dsi] if _dsi is not None else np.nan
                    if _dp == _dp:                      # not NaN -> filter active
                        if daytype_mode == "skip_bot_short" and _dp <= daytype_lo:
                            short_ok = False
                        elif daytype_mode == "skip_bot_all" and _dp <= daytype_lo:
...
# -----------------------------------------------------------------------------
# Smoke test: (1) daytype_mode='off' must be BIT-IDENTICAL to ORB_3_6.py on the
# run #234 crowned params; (2) each ON mode must actually change results.
# Run:  python augur_strategies/ORB_3_8.py
# -----------------------------------------------------------------------------
if __name__ == "__main__":
    ...
    if not os.path.exists(MASTER):
        print("NQ master not found"); sys.exit(1)
    ...
    base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ORB_3_6.py")
    spec = ilu.spec_from_file_location("_orb36", base_path)
    base = ilu.module_from_spec(spec); spec.loader.exec_module(base)

    C2 = dict(or_bars=2, trade_mode="First-candle dir", stop_frac=2.0, breakout_buf=0.25,
    ...
    r0 = base.run_backtest(*args, **kw, **C2)
    r1 = run_backtest(*args, **kw, **C2, daytype_mode="off")
    same = (r0["num_trades"] == r1["num_trades"]
            and abs(r0["total_pnl"] - r1["total_pnl"]) < 1e-9
            and all(a == b for a, b in zip(r0["trades"], r1["trades"])))
    print("daytype OFF parity vs ORB_3_6: %s  (n %d vs %d, pnl %.6f vs %.6f)" % (
