"""
ENGUD-Q 1m · trendline break (short)
------------------------------------
Exact short mirror of ENGU-Q (Engulfing Down - Quantitative, owner's name). Same
engine as ENGUQ_1M_1_0.py with every comparison flipped: an ASCENDING trendline
fit to recent LOWS (line of higher-lows) instead of a descending one fit to
highs, broken DOWNWARD by a red candle instead of broken upward by a green one.
Gap-honest fills (fill = the open when it gapped through the stop, never a
price the market didn't trade at), same breakeven_R + trailing-stop scaffold.

Entry (short): downtrend (close<EMA), RED candle CLOSING BELOW an ascending
trendline fit to the last tl_len lows (breaks the line of higher-lows = the
bounce), below the prior low, on a volume spike, decisive break. Stop = swing
high; exit = trailing stop. Trades are returned rich:
(entry_idx, exit_idx, pnl_pts, side=-1, entry_px) -> MAE/MFE.
"""
import numpy as np

STRATEGY_NAME = "ENGUD-Q 1m · trendline break (short)"
DESCRIPTION = ("ENGUD-Q: the short mirror of ENGU-Q. Red candle breaking an ascending trendline of recent higher-lows, below the trend EMA, on a volume spike; trailing exit. Gap-honest fills.")
VERSION   = "1.0"
DIRECTION = "SHORT"
TIMEFRAME = "1m"
...
DEFAULT_PARAMS = {'tl_len': {'default': 34,
          ...
          'tooltip': 'Bars of lows the ascending trendline is fit to (must slope up). 1m: try ~30-70 '
                     '(tl_len 34 validated on the long).'},
'vol_mult': {'default': 1.2,
            ...
            'tooltip': 'Breakdown candle volume must exceed its 20-bar average x this. 0=off.'},
'stop_mult': {'default': 1.7,
             'min': 0.3,
             'max': 2.0,
             'step': 0.1,
             'type': 'float',
             'label': 'Stop (x risk-to-swing-high)',
             'tooltip': 'Initial stop distance as a fraction of entry-to-swing-high.'},
'act_R': {'default': 1.0,
         'min': 0.0,
         'max': 3.0,
         'step': 0.5,
...
'trail_frac': {'default': 2.5,
              ...
              'tooltip': 'Trailing stop rides this far (in risk units) above the running low.'},
'buf_atr': {'default': 0.35,
           'min': 0.0,
           'max': 1.0,
           'step': 0.05,
           'type': 'float',
           'label': 'Breakdown Buffer (x ATR)',
           'tooltip': 'Close must clear the trendline (below) by this x ATR.'},
'min_brk': {'default': 0.7,
           'min': 0.0,
           'max': 3.0,
           'step': 0.1,
           'type': 'float',
           'label': 'Breakdown Decisiveness (x ATR)',
           'tooltip': 'Trendline-minus-close must be at least this x ATR (a decisive break).'},
'ema_len': {'default': 30,
           'min': 20,
           'max': 400,
           'step': 10,
           ...
           'tooltip': 'Only take shorts with close below this EMA (downtrend filter).'},
'atr_len': {'default': 47,
           'min': 5,
           'max': 50,
           'step': 1,
...
'regime_len': {'default': 0,
              ...
              'tooltip': 'Only go short when close is below its N-DAY simple average — a longer-term trend gate on '
                         'top of the EMA. 0=off. Long (~20-50d) skips bulls/chop and cuts drawdown; short whipsaws. '
                         '(390 RTH bars/day.)'},
'breakeven_R': {'default': 0.0,
               ...
               'tooltip': 'Once the trade is this many R in profit (low vs entry), lower the stop to entry so it '
                          'can never go negative. 0=off. 1.5 tested well on the long: protects the 1.5R-2.5R window '
                          'before the trail activates without scratching the fat-tail winners.'}}

PARAM_GRID_PRESETS = {'Robust (high-frequency)': {'tl_len': [25, 34, 45, 60],
                           'vol_mult': [1.1, 1.2, 1.3],
                           'stop_mult': [1.0, 1.5, 1.7],
                           'act_R': [0.5, 1.0, 1.5, 2.5],
                           'trail_frac': [1.0, 1.5, 2.0, 2.5],
                           'buf_atr': [0.2, 0.35],
                           'min_brk': [0.3, 0.7],
                           'ema_len': [30, 100, 200],
                           'atr_len': [14, 47],
...
def run_backtest(opens, highs, lows, closes, volumes=None, day_id=None,
    ...
    # optional longer-term REGIME gate: close must be below its N-day simple average
    #   (390 RTH bars/day). 0 = off. Long lengths skip bulls/chop; short lengths whipsaw.
    reg = None
    if int(regime_len) > 0:
    ...
    for i in range(tl_len + 1, n):
        ...
        if not (c[i] < tl_now - buf_atr * a and c[i] < l[i - 1]):
            continue
        if (tl_now - c[i]) / max(a, 0.25) < min_brk:
            continue
    ...
    if pos is not None:
        pnl = pos["ep"] - c[-1]; pnl_list.append(pnl)
