"""
NOISE 1.1 N304 -- the live crown's settings, run on a different bar (validated here on 15-minute bars).
...
This configuration was measured there but has never been the CHAMPION of an Auto-Validate:
  round 44 measured the crown's settings on 15-minute bars by hand; run #362 searched 15-minute bars
  wide open and crowned a different configuration, so these settings on that bar were never judged.
...
WHAT THIS FILE IS
  ...
  (15-minute bars, tape ends 2026-06-30) 3,011 trades, profit factor 1.446, 2024 onward 1.356.
...
_base = _load("NOISE_1_0.py", "_n10_for_noise_1_1_n304")

STRATEGY_NAME = "NOISE 1.1 live crown neighbourhood (confirm 1)"
DESCRIPTION = ("The live NOISE crown (run 304) with its five numeric knobs open one step either side and every other setting frozen, confirmation fixed at one close. Built to validate the crown settings on another bar.")
