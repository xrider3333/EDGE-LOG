"""
ORB 3.6 OS - the crown with a DELAYED opening range (round 10, 2026-09-08).

Every ORB sweep so far varied the LENGTH of the opening range (or_bars) but never its
START: the range always begins on the session's first bar (9:30 ET). This fork adds ONE knob:

  * or_start_bar - the range starts this many bars into the session (0 = the parent).
                   On 5m RTH bars 3 = 9:45, 6 = 10:00 (covers the 10:00 data releases),
                   9 = 10:15, 12 = 10:30. The first-candle direction, breakout levels,
                   stop/target/BE geometry and the vpace/atr filters are all unchanged and
                   measured from the delayed range. Bars before the range are ignored.

Knob at 0 is asserted bit-identical to ORB_3_6 (smoke test below). Live-legal: reads only
finished bars, like the parent.
"""
import numpy as np

STRATEGY_NAME = 'ORB 3.6 OS · legal base + breakeven'
...
DEFAULT_PARAMS = {
    ...
    "or_start_bar": {
        "default": 0, "type": "int", "min": 0, "max": 18, "step": 3,
        "label": "Opening range starts N bars into the session",
        "tooltip": "0 = range from the 9:30 open (the parent). 6 on 5m bars = the range is 10:00-10:10.",
    },
}
...
def run_backtest(
    ...
    while i < n:
        ...
        if _allow_start.get(i, True) is False:   # vol-regime filter skipped this session
            i = j; continue
        s0 = int(or_start_bar)
        if m > s0 + or_bars + 1 and or_bars >= 1:
            so, sh, sl, sc = o[i:j], h[i:j], l[i:j], c[i:j]
            sv = v[i:j] if v is not None else None
            or_hi = sh[s0:s0 + or_bars].max()
            or_lo = sl[s0:s0 + or_bars].min()
            rng   = or_hi - or_lo
            if rng > 0:
                or_dir = 1 if sc[s0 + or_bars - 1] >= so[s0] else -1
