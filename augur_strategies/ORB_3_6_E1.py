"""
ORB 3.6 — ENTRY VARIANT E1: most money + best time-consistency.
...
6 of 192 survived. This file is one of the three distinct survivors.
    ...
    entry: OR 2 - first-candle dir - close-confirm - buf 0.30 - v-pace 0.7 - ATR 0.5
    stop:  2.50 x range width        exit: ride to 5.5R, breakeven at 1.0R

Offline (NQ 5m RTH no-adj, 0.533 pts/RT, 1 contract, run #230's window; lockbox measured
with a REAL windowed run, not a slice):
    full net $416,381  -  maxDD $32,505  -  net/DD 12.81
    lockbox $91,404 at PF 1.502  -  rolling-12mo win 75.4%, worst -$19,961
...
_spec = _ilu.spec_from_file_location("_orb36_base_e1", _SRC)
_base = _ilu.module_from_spec(_spec)
_spec.loader.exec_module(_base)

STRATEGY_NAME = 'ORB 3.6 · entry variant E1 — most money + best time-consistency, pinned'
DESCRIPTION   = ("First re-optimisation of the ENTRY under the new ride+BE exit. "
                 ...
                 "(most money + best time-consistency). Judge on WF folds vs run #234's 7/8, not the offline net.")
