"""
OPENING RANGE BREAKOUT 3.7 — 3.6 (legal base + breakeven) + RE-ENTRY AFTER A SCRATCH.
...
STRATEGY_NAME = 'ORB 3.7 · 3.6 + re-entry after BE scratch'
...
DEFAULT_PARAMS = {
    ...
    "reenter_scratch": {
        ...
        "label": "Re-entries after a BE scratch (0=off)",
        "tooltip": "After the breakeven stop takes you out AT ENTRY OR BETTER (a scratch, "
                   "not a loss), allow this many fresh entries in the same session on the "
                   "next bar that satisfies ALL the usual entry conditions again. Item Q's "
                   "re-entry-after-LOSS was dead; a re-entry after a SCRATCH is a different "
                   "population that only exists once breakeven is on. 0 = off (= 3.6).",
    },
    ...
    while i < n:
        ...
        if m > or_bars + 1 and or_bars >= 1:
            ...
            if rng > 0:
                ...
                reent_left = int(reenter_scratch)
                    ...
                    if pos == 0:
                        ...
                        if pos > 0:
                            if sl[k] <= stop:                       # stop first (pessimistic)
                                ...
                                pos = 0
                                # 3.7: a BE SCRATCH (exit at entry or better) may re-arm
                                # the session for a fresh entry from the NEXT bar on.
                                if reent_left > 0 and be_armed and ex_px >= entry - 1e-9:
                                    reent_left -= 1
                                    be_armed = False
                                    continue
                                break
