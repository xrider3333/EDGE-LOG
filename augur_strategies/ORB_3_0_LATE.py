"""
OPENING RANGE BREAKOUT — 137-LOCK + BREAKEVEN + SESSION-AGE-SCALED STOP (X14).

Byte-for-byte the ORB_3_0_BE.py deploy champion (or1 / Both / stop 1.75 / target
4.5R / vol 1.25 / atr 0.1 / be_after_R 1.0) with ONE addition: params `late_bar`
(int, default 0 = off) and `late_stop_frac` (float, default 1.0). When
late_bar > 0 and the ENTRY fires at session-bar index >= late_bar, THAT trade's
stop uses late_stop_frac x OR-range instead of stop_frac. Risk, target and
breakeven all key off the actual stop used (target_R x its own risk, be_after_R
x its own risk) — the whole R-ladder rescales with the late stop.

Rationale (P2): late breaks are 63% of trades at PF 1.30 — a wide 1.75x stop
sized for the 9:35 breakout may be mis-sized for a 11:00+ breakout with less
session left to pay for it. Causality: the session-bar index of the entry touch
is known AT the touch — nothing from the entry bar beyond the touch is used.
late_bar=0 = EXACT ORB_3_0_BE behavior (anchor: deploy full = $574,177 /
DD $26,763 / 3951 trades). PNL = SHARES*(EXIT-ENTRY).
"""
import numpy as np

STRATEGY_NAME = 'ORB 3.0 · 137-lock + BE + late-entry stop scale'
DESCRIPTION   = ("The ORB_3_0_BE deploy champion with one extra lever: entries firing at "
                 "session-bar index >= late_bar use late_stop_frac (x OR range) instead of "
                 "stop_frac, with target and breakeven rescaled off that trade's own risk. "
                 "0 = off (= exact deploy).")
...
DEFAULT_PARAMS = {
    ...
    "late_bar": {
        "default": 0, "min": 0, "max": 48, "step": 4, "type": "int",
        "label": "Late-entry threshold (session bar, 0=off)",
        "tooltip": "Entries firing at session-bar index >= this switch to late_stop_frac. "
                   "16 = 10:50 ET on 5m RTH; 24 = 11:30 ET. 0 = off (= exact ORB_3_0_BE).",
    },
    "late_stop_frac": {
        "default": 1.0, "min": 0.5, "max": 2.0, "step": 0.25, "type": "float",
        "label": "Late-entry stop (× range width)",
        "tooltip": "Stop fraction used for late entries (session bar >= late_bar). Target "
                   "and breakeven rescale off this trade's own risk. Inert while late_bar=0.",
    },
}

PARAM_GRID_PRESETS = {
    "Late-stop sweep (137+BE locked)": {
        "or_bars": [1], "trade_mode": ["Both"], "stop_frac": [1.75], "vol_filter": [1.25],
        "atr_filter": [0.1], "breakout_buf": [0.0], "target_R": [4.5], "be_after_R": [1.0],
        "late_bar": [0, 16, 24], "late_stop_frac": [1.0, 1.25, 1.5],
    },
}
...
def run_backtest(
    ...
    late_bar: int = 0, late_stop_frac: float = 1.0,
    ...
    #    Trailing-only windows, no look-ahead (see ORB_3_0_BE.py for the full note).
    _sess_bounds = []
    _a = 0
    while _a < n:
        _b = _a
    ...
    # ── Half-day / holiday skip: sessions shorter than 70% of the median length.
    _holiday_start = set()
                    ...
                    if pos == 0:
                        ...
                        if vol_filter > 0 and sv is not None and k > 0:
                            ...
                            if mv > 0 and sv[k] < vol_filter * mv:
                                continue
                        # X14: session-age-scaled stop — an entry firing at session-bar
                        # index >= late_bar uses late_stop_frac. k is known at the touch
                        # (causal); target/BE below key off (entry - stop) = this trade's
                        # own risk, so the whole R-ladder rescales automatically.
                        _sf = late_stop_frac if (late_bar > 0 and k >= late_bar) else stop_frac
                        if long_ok and up:
                        ...
                        if pos > 0:
                            ...
                            if target_R > 0 and sh[k] >= tgt:
                                ...
                                pos = 0; break
                            # Breakeven: arm on this bar's CLOSE once high-water >= X × risk;
                            # takes effect next bar (causal). Checked AFTER stop/target.
