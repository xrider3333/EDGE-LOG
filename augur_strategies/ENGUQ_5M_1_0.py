"""
ENGU-Q 5m · trendline break (long)
----------------------------------
Timeframe-branched ENGU-Q (5m). One of three TF-specific strategies (1m/5m/15m) so
...
STRATEGY_NAME = "ENGU-Q 5m · trendline break (long)"
DESCRIPTION = ("ENGU-Q tuned for 5-minute bars: green candle breaking a descending trendline of recent lower-highs, above the 200-EMA, on a volume spike; trailing exit. Robust high-frequency neighbourhood on ES 5m.")
VERSION   = "1.0"
DIRECTION = "LONG"
TIMEFRAME = "5m"

DEFAULT_PARAMS = {'tl_len': {'default': 25,
          'min': 10,
          'max': 60,
          'step': 1,
          'type': 'int',
          'label': 'Trendline Length (bars)',
          'tooltip': 'Bars of highs the descending trendline is fit to (must slope down). 5m: try ~15-30 '
                     '(tl_len 25 robust).'},
...
PARAM_GRID_PRESETS = {'Robust (high-frequency)': {'tl_len': [15, 20, 25, 30],
...
'Wide': {'tl_len': [10, 15, 20, 25, 30, 40],
        'vol_mult': [1.2, 1.3, 1.5, 2.0],
        'stop_mult': [0.8, 1.0, 1.2],
        'act_R': [1.0, 1.5],
