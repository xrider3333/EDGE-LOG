"""
ENGU-Q 1m ETH · SHALLOW LIMIT 0.20 ATR — FROZEN (validate card)
-----------------------------------------------------------------
Pinned single-point fork of ENGUQ_1M_ETH_LIM_1_0.py (the shallow-limit engine) with
limit_atr fixed at 0.20 and every other knob pinned to the certified #226 ETH config.
...
_spec = _ilu.spec_from_file_location("_enguq_eth_lim_base_20", _SRC)
_base = _ilu.module_from_spec(_spec)
_spec.loader.exec_module(_base)

STRATEGY_NAME = "ENGU-Q 1m ETH LIMIT 0.20 FROZEN 1.0"
DESCRIPTION = ("Certified #226 ETH config with a resting limit entry 0.20 x ATR below the "
...
DEFAULT_PARAMS = {
    "limit_atr":   {"default": 0.20, "min": 0.20, "max": 0.20, "step": 0.05,"type": "float", "label": "Shallow Limit Depth (x ATR) — PINNED"},
...
PARAM_GRID_PRESETS = {
    "PINNED - 226 config + limit 0.20 ATR": {
        "limit_atr":   [0.20],
        "tl_len":      [170],
        "vol_mult":    [0.8],
        "stop_mult":   [1.0],
