...
# -- Identity -----------------------------------------------------------------
STRATEGY_NAME = 'ENGU 1.1.21 · daily bias + flat-EOD (GROK)'
DESCRIPTION   = (
...
DEFAULT_PARAMS = {
    ...
    "bias_mode": {
        ...
        "tooltip": "Long-only, so only enter when price is ABOVE a daily anchor — "
                   "don't buy into a down-day. prior_close = yesterday's close; "
                   "session_open = today's open (gap-immune); off = no bias. Needs session data.",
    },
    "flat_eod": {
        "default": False, "type": "bool",
        "label": "Flat by Session Close",
        "tooltip": "Close any open position at each session's last bar (no overnight "
                   "holds). Needs session data.",
    },
}
...
def run_backtest(
    ...
    if day_id is not None and len(day_id) == n and (bias_mode != "off" or flat_eod):
        ...
        for _j in range(n):
            if _j == 0 or _did[_j] != _did[_j - 1]:
                ...
                _cur_open = opens[_j]
            _pclose[_j] = _prev_last; _sopen[_j] = _cur_open
    ...
    for i in range(start_idx, n):
        ...
        # Directional bias (long-only): only enter when above the daily anchor,
        # so we don't buy into a down-day. Off = original behaviour.
        if bias_mode != "off" and _pclose is not None:
            _ref = _pclose[i] if bias_mode == "prior_close" else _sopen[i]
            if not np.isnan(_ref) and closes[i] <= _ref:
                continue

        # Don't open on a session's last bar (flat-by-close would instantly exit it).
        if flat_eod and _send is not None and _send[i]:
            continue
