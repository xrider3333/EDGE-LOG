...
# -- Identity -----------------------------------------------------------------
STRATEGY_NAME = 'ENGU 1.3.5 · daily bias + flat-EOD (CLAUDE)'
DESCRIPTION   = (
...
DEFAULT_PARAMS = {
    ...
    "bias_mode": {
        ...
        "tooltip": "Only take LONGS when price is above a daily anchor, SHORTS when "
                   "below — so you don't fight the day's direction. 'prior_close' = "
                   "the previous session's close; 'session_open' = today's open "
                   "(gap-immune); 'off' = no bias filter. Needs session data.",
    },
    "flat_eod": {
        "default": False, "type": "bool",
        "label": "Flat by Session Close",
        "tooltip": "Close any open position at each session's last bar (no overnight "
                   "holds) — a true day-trading model; also removes end-of-data "
                   "force-close artifacts. Needs session data.",
    },
}
...
def run_backtest(
    ...
    # Session anchors derived from a per-bar session id the app passes (the strategy
    # gets no timestamps, so it can't find day boundaries itself). _pclose = previous
    # session's close, _sopen = current session's open, _send = True on each session's
    # last bar. All None when no session id is given → bias filter + flat-by-close are
    ...
    if day_id is not None and len(day_id) == n and (bias_mode != "off" or flat_eod):
        ...
        _send = np.zeros(n, dtype=bool)
        _prev_last = np.nan; _cur_open = np.nan
        for _j in range(n):
            if _j == 0 or _did[_j] != _did[_j - 1]:
                if _j > 0:
                    _prev_last = closes[_j - 1]
                _cur_open = opens[_j]
            _pclose[_j] = _prev_last
            _sopen[_j] = _cur_open
            _send[_j] = (_j == n - 1) or (_did[_j + 1] != _did[_j])
    ...
    start_idx = max(lookback_bars + 1, atr_length if use_atr else 0,
                    ...
                    ema_length if use_trend else 0, opp_lookback + 1) + 1
    ...
    for i in range(start_idx, n):
        ...
        if pos is not None:
            ...
            # Flat by session close — exit any open position at the session's last
            # bar (no overnight holds; also removes the end-of-data force-close spike).
            if flat_eod and _send is not None and _send[i]:
                pnl = (closes[i] - pos["ep"]) if pos["side"] > 0 else (pos["ep"] - closes[i])
                pnl_list.append(pnl)
                if return_trades: trade_log.append((pos["bar"], i, pnl))
                last_loss_bar = (i if pnl < 0 else last_loss_bar)
                pos = None; continue
        ...
        # Directional bias: only long above a daily anchor, only short below it — so
        # we don't fight the day's direction. prior_close anchor is gap-prone, the
        # session_open anchor is gap-immune; the optimizer picks which (or off).
        bias_long = bias_short = True
        if bias_mode != "off" and _pclose is not None:
            _ref = _pclose[i] if bias_mode == "prior_close" else _sopen[i]
            if not np.isnan(_ref):
                bias_long = closes[i] > _ref
                bias_short = closes[i] < _ref

        bull_sig = raw_bull and opp_pass_bull and atr_pass and vol_pass and trend_bull and bias_long
        bear_sig = raw_bear and opp_pass_bear and atr_pass and vol_pass and trend_bear and bias_short
        ...
        # Don't open a new trade on a session's last bar — it'd be force-flat
        # immediately, with no room to play out.
        if flat_eod and _send is not None and _send[i]:
            continue
