...
WHY THIS FORK EXISTS (2026-08-08): every TTIBS Auto-Validate ever run (#161, #162,
...
This file removes "close" from the fill_mode options entirely, so no search run on it
can ever crown the leak again. Everything else - triggers, exits, hold cap, ma200
regime gate, the roll-seam guard, the end-of-data drop rule - is byte-identical to
TTIBS 1.0. Full ranges are kept on every knob (Auto-Validate must search a real
surface, never a pinned config).
...
STRATEGY_NAME = 'TTIBS 1.1 · IBS mean reversion, HONEST FILL (long)'
DESCRIPTION = ("HONEST-FILL fork of TTIBS 1.0 - the look-ahead same-close fill is removed, "
               "so a search can never crown it again (all ten prior TTIBS runs did). "
               "Daily buy-weakness mean reversion, long only: Monday-selloff or "
