"""NOISE 1.7 EXIT -- research fork of NOISE_1_0.py adding the two exit-management devices
the other two crowned families both use and this one never had: a BREAKEVEN move and a
TRAILING stop, both expressed in multiples of the trade own initial risk.

The parent sets its protective stop once at entry and never moves it. ORB rides with a
breakeven and a trail; ENGU-Q has a breakeven and a trail. NOISE has neither, and nobody
ever tested whether it should. be_r=0 and trail_frac=0 reproduce the parent EXACTLY
(parity-gated in tools/r52_noise_exit_management.py).

Both devices read the PREVIOUS bar completed extreme, never the bar being traded.

Research only. Nothing on any board runs this file.
"""
_PARENT_DOC = """
...
STRATEGY_NAME = 'NOISE 1.7 EXIT - breakeven and trailing stop as knobs (research)'
...
DEFAULT_PARAMS = {
    "lookback": {
        "default": 40, "min": 40, "max": 40, "step": 1, "type": "int",
    ...
    "band_mult_long": {
        "default": 0.75, "min": 0.75, "max": 0.75, "step": 0.25, "type": "float",
    ...
    "band_mult_short": {
        "default": 1.5, "min": 1.5, "max": 1.5, "step": 0.25, "type": "float",
    ...
    "flat_eod": {
        "default": True, "type": "bool", "options": [True],
    ...
    "skip_holidays": {
        "default": False, "type": "bool", "options": [False],
    ...
    "confirm_bars": {
        "default": 1, "min": 1, "max": 1, "step": 1, "type": "int",
    ...
    "daytype_mode": {
        "default": "skip_bot_short", "type": "str",
        "options": ["skip_bot_short"],
    ...
    "daytype_lo": {
        "default": 0.2, "min": 0.2, "max": 0.2, "step": 0.05, "type": "float",
    ...
    "daytype_hi": {
        "default": 0.8, "min": 0.8, "max": 0.8, "step": 0.05, "type": "float",
    ...
    "be_r": {
        ...
        "tooltip": "Once the trade has run this many multiples of its own initial risk in "
                   "your favour, move the protective stop up to the entry price so the trade "
                   "can no longer lose. 0 = off, which is what the family has always done. "
                   "Measured on the PREVIOUS bar completed extreme, never the current one.",
    },
    "trail_frac": {
        "default": 0.0, "min": 0.0, "max": 4.0, "step": 0.25, "type": "float",
        "label": "Trail width (x initial risk, 0=off)",
        "tooltip": "Once the trade has run one multiple of its initial risk, keep the stop "
                   "this many multiples behind the best price seen so far. 0 = off. The stop "
                   "only ever moves in your favour, and it is computed from finished bars.",
    },
    "vol_skip_pct": {
        "default": 95.0, "min": 95.0, "max": 95.0, "step": 1.0, "type": "float",
    ...
    "stop_k": {
        "default": 1.75, "min": 1.75, "max": 1.75, "step": 0.25, "type": "float",
...
def run_backtest(
    ...
    be_r: float = 0.0, trail_frac: float = 0.0,
    day_id=None,
    ...
    for si, (a, b) in enumerate(sess_bounds):
        ...
        entry_risk = 0.0      # |entry - initial stop|, the R unit for be_r/trail_frac
        best_px = 0.0         # best price seen since entry, for the trail
        ...
        for k in range(m):
            ...
            if entry_pending != 0 and pos == 0:
                ...
                # EXIT MANAGEMENT (2026-09-09 round 52). Both devices are expressed in R,
                # where R is the distance from the entry to the INITIAL stop -- the same
                # convention ORB and ENGU-Q use. Zero on either knob leaves every code path
                # below untouched, so the file is byte-identical to the parent when off.
                entry_risk = abs(entry_px - stop_level) if stop_level is not None and not np.isnan(stop_level) else 0.0
                best_px = entry_px
            ...
            # BREAKEVEN and TRAIL, evaluated on the PREVIOUS bar's finished extreme so the
            # level is known before this bar trades -- never on the bar that sets the extreme,
            # which would be reading the future (the look-ahead this program has twice paid for).
            if pos != 0 and k > entry_k + 1 and entry_risk > 0.0 and stop_level is not None:
                if pos > 0:
                    best_px = max(best_px, sh[k - 1])
                    run_r = (best_px - entry_px) / entry_risk
                    if be_r > 0.0 and run_r >= be_r:
                        stop_level = max(stop_level, entry_px)
                    if trail_frac > 0.0 and run_r >= 1.0:
                        stop_level = max(stop_level, best_px - trail_frac * entry_risk)
                else:
                    best_px = min(best_px, sl[k - 1]) if best_px != entry_px else min(entry_px, sl[k - 1])
                    run_r = (entry_px - best_px) / entry_risk
                    if be_r > 0.0 and run_r >= be_r:
                        stop_level = min(stop_level, entry_px)
                    if trail_frac > 0.0 and run_r >= 1.0:
                        stop_level = min(stop_level, best_px + trail_frac * entry_risk)
