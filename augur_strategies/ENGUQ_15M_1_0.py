"""
ENGU-Q 15m · trendline break (long)
-----------------------------------
Timeframe-branched ENGU-Q (15m). One of three TF-specific strategies (1m/5m/15m) so
...
STRATEGY_NAME = "ENGU-Q 15m · trendline break (long)"
DESCRIPTION = ("ENGU-Q tuned for 15-minute bars: green candle breaking a descending trendline of recent lower-highs, above the 200-EMA, on a volume spike; trailing exit. Higher timeframe; fewer, larger signals on ES 15m.")
VERSION   = "1.0"
DIRECTION = "LONG"
TIMEFRAME = "15m"

DEFAULT_PARAMS = {'tl_len': {'default': 20,
          'min': 8,
          'max': 50,
          'step': 1,
          'type': 'int',
          'label': 'Trendline Length (bars)',
          'tooltip': 'Bars of highs the descending trendline is fit to (must slope down). 15m: try ~12-34 '
                     '(tl_len 20 robust).'},
'vol_mult': {'default': 1.3,
            'min': 0.0,
            'max': 5.0,
            'step': 0.1,
            'type': 'float',
...
PARAM_GRID_PRESETS = {'Robust (high-frequency)': {'tl_len': [12, 15, 20, 25],
...
'Wide': {'tl_len': [8, 12, 15, 20, 25, 34],
        'vol_mult': [1.2, 1.3, 1.5],
        'stop_mult': [0.8, 1.0, 1.5],
        'act_R': [1.0, 1.5],
        'trail_frac': [1.5, 2.0, 2.5],
        'buf_atr': [0.1, 0.25, 0.35],
        'min_brk': [0.0, 0.5, 0.7],
        'ema_len': [30, 100, 200],
        'atr_len': [14, 47],
...
def run_backtest(opens, highs, lows, closes, volumes=None, day_id=None,
                 ...
                 buf_atr=0.1, min_brk=0.5, ema_len=200, atr_len=14,
                 breakeven_R=0.0,
