"""
ORB 3.6 FILL - the crown with a REALISTIC entry fill (2026-09-08, "continue improvements").

ORB_3_6 fills a close-confirmed breakout AT THE CLOSE of the confirming bar. Live, a
market order fired on that close fills at the NEXT bar's open - you cannot buy the print
that told you to buy. On a 5-minute chart the gap between the two is usually a tick or two
and sometimes an air pocket. This fork adds ONE knob:

  * fill_next_open  - fill at the open of the bar after the confirming close (default
                      OFF). The stop, target and breakeven are all measured from that
                      real fill, and the fill bar itself is managed (its low can hit the
                      stop). If the confirming bar is the session's last, no trade.

With the knob off this file is asserted bit-identical to ORB_3_6 (smoke test at the
bottom). LIVE-LEGAL by construction: it reads LESS of the future than the parent.
"""
import numpy as np

STRATEGY_NAME = 'ORB 3.6 FILL · legal base + breakeven'
...
DEFAULT_PARAMS = {
    ...
    "fill_next_open": {
        ...
        "label": "Fill at the NEXT bar's open (realistic)",
        "tooltip": "OFF = fill at the confirming bar's close (the parent's assumption). ON = fill "
                   "at the open of the following bar, which is what a market order on the close gets.",
    },
}
                ...
                for k in range(or_bars, m):
                    if pos == 0:
                        ...
                        if long_ok and up:
                            ...
                            if fill_next_open:
                                if k + 1 >= m:
                                    break            # confirmed on the last bar: nothing to fill into
                                entry = so[k + 1]
                            risk  = stop_frac * rng
                            stop  = entry - risk
                            ...
                            pos = 1; ek = (k + 1) if fill_next_open else k; p_done = False; p_pnl = 0.0; be_armed = False; continue
...
if __name__ == "__main__":
    ...
    assert same
    c = run_backtest(O, H, L, C, **A, **dict(P, fill_next_open=True))
    print("fill_next_open: %d trades, $%.0f  (close-fill: %d trades, $%.0f)"
          % (c["num_trades"], c["total_pnl"] * 20, a["num_trades"], a["total_pnl"] * 20))
    print("ORB_3_6_FILL smoke: PASS")
