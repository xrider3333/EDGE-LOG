"""NOISE 1.5 ANCHOR -- research fork of NOISE_1_0.py that makes the BAND ANCHOR a knob.

The shipped family measures its bands from a level fixed for the whole session -- the max
and min of the session open and the prior close -- so a 15:00 entry is judged against a
morning level. This fork adds anchor_mode: "open" reproduces the parent EXACTLY (parity
gated in tools/r49_noise_anchor.py), "vwap" anchors on the session running VWAP through the
previous bar, "mid" on the midpoint of the session range so far. Everything else, including
the sigma estimator and every filter, is the parent unchanged.
...
STRATEGY_NAME = 'NOISE 1.6 SIGNALS - every trigger logged, taken or not (research)'

# Round 50 instrumentation: every bar where the entry trigger fires is appended here as
# (bar_index, side, position_open). Behaviour is UNCHANGED - the trigger is still only
# ACTED on when flat; this records the ones the single-position rule throws away.
SIGNAL_LOG = []
...
DEFAULT_PARAMS = {
    ...
    "anchor_mode": {
        "default": "open", "type": "str", "options": ["open", "vwap", "mid"],
        "label": "Band anchor",
        "tooltip": "What the bands are measured FROM. open (shipped) = a level fixed for the "
                   "whole session, max/min of the session open and the prior close. vwap = the "
                   "session running VWAP through the PREVIOUS bar. mid = the midpoint of the "
                   "session range so far, again through the previous bar. Both alternatives let "
                   "the band follow the session instead of staying anchored to the morning.",
    },
    "vol_skip_pct": {
...
def run_backtest(
    ...
    for si, (a, b) in enumerate(sess_bounds):
        ...
        # ANCHOR (research knob, 2026-09-09 round 49). 'open' is the shipped behaviour and
        # is byte-identical; the alternatives replace the FIXED session anchor with one that
        # moves as the session develops. Both alternatives are causal: at bar k they use only
        # bars 0..k-1, and the value at index 0 falls back to the shipped anchor.
        ref_hi = max(so[0], prev_close)
        ref_lo = min(so[0], prev_close)
        ref_hi_arr = ref_lo_arr = None
        if anchor_mode in ('vwap', 'mid'):
            if anchor_mode == 'vwap' and sv is not None:
                _typ = (sh + sl + sc) / 3.0
                _cv = np.cumsum(sv)
                with np.errstate(invalid='ignore', divide='ignore'):
                    _a = np.cumsum(_typ * sv) / _cv
            else:
                _a = (np.maximum.accumulate(sh) + np.minimum.accumulate(sl)) / 2.0
            _a = np.r_[ref_hi, _a[:-1]]          # shift: bar k sees bars 0..k-1 only
            _a = np.where(np.isnan(_a), ref_hi, _a)
            ref_hi_arr = np.maximum(_a, 0.0)
            ref_lo_arr = ref_hi_arr
        sigma_row = sigma[si, :]
        with np.errstate(invalid="ignore"):
            _rh = ref_hi if ref_hi_arr is None else ref_hi_arr[:m]
            _rl = ref_lo if ref_lo_arr is None else ref_lo_arr[:m]
            UB = _rh * (1.0 + band_mult_long * sigma_row[:m])
            LB = _rl * (1.0 - band_mult_short * sigma_row[:m])
        ...
        for k in range(m):
            ...
            if not is_last and 1 <= k <= m - 2 and not sess_block_entries:
                in_window = True
                if window == "morning":
                    in_window = (k <= 29)
                ...
                if in_window:
                    ...
                    if long_trig or short_trig:
                        _sd = 1 if (long_trig and not short_trig) else (
                            -1 if (short_trig and not long_trig) else
                            (1 if (sc[k] - ub_k) >= (lb_k - sc[k]) else -1))
                        SIGNAL_LOG.append((a + k, _sd, 1 if pos != 0 else 0))
                    if pos == 0:
