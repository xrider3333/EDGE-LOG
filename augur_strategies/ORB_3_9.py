"""
ORB 3.9 — EXIT MANAGEMENT ROUND 2 on the run #234 crown (2026-08-22 hunt).

Three levers the voided touch-entry record explored but the LEGAL base never has.
All default OFF; with all three off this file is asserted bit-identical to ORB_3_6.

  * reenter_stop   — after a REAL stop-out loss (>= half the initial risk lost), keep
                     scanning the session for a fresh breakout signal, at most this many
                     times. Distinct from ORB_3_7 (re-entry after a breakeven SCRATCH,
                     measured and dead); this asks about loss days instead.
  * be_after_bars  — arm breakeven after N bars in the trade regardless of R progress
                     (the old item-K "time trigger", never tested on the legal base).
                     Arms on a finished bar, acts next bar, same as the R trigger.
  * be_lock_frac   — when breakeven arms, lock the stop ABOVE entry by this fraction of
                     initial risk (pays the round-turn instead of scratching to zero).

LIVE-LEGAL: nothing new reads the current bar before it exists — arming still happens on
finished closes and acts from the next bar; re-entry re-uses the ordinary entry logic.
"""
import numpy as np

STRATEGY_NAME = 'ORB 3.9 · exit management round 2'
...
DEFAULT_PARAMS = {
    ...
    "reenter_stop": {
        ...
        "label": "Re-enter after a real stop-out (0=off)",
        "tooltip": "After an exit that lost at least half the initial risk, keep scanning "
                   "the session for a fresh breakout signal, at most this many more times. "
                   "0 = one loss ends the session (the crown's behaviour).",
    },
    "be_after_bars": {
        "default": 0, "min": 0, "max": 24, "step": 2, "type": "int",
        "label": "Arm breakeven after N bars (0=off)",
        "tooltip": "Arm the breakeven stop once the trade is N bars old, whatever the "
                   "price has done. Arms on a finished bar and acts from the next bar, "
                   "exactly like the R trigger. Stacks with be_after_R.",
    },
    "be_lock_frac": {
        "default": 0.0, "min": 0.0, "max": 0.5, "step": 0.05, "type": "float",
        "label": "Breakeven lock (x risk above entry, 0=plain)",
        "tooltip": "When breakeven arms, place the stop this fraction of initial risk "
                   "BEYOND entry instead of exactly at it - a scratch then pays the "
                   "round turn instead of losing it.",
    },
    "flat_eod": {
        "default": True, "type": "bool",
...
def run_backtest(
    ...
    reenter_stop: int = 0, be_after_bars: int = 0, be_lock_frac: float = 0.0,
    ...
    while i < n:
        ...
        if m > or_bars + 1 and or_bars >= 1:
            ...
            if rng > 0:
                ...
                be_armed = False; be_lvl = np.nan; _stops_taken = 0
                for k in range(or_bars, m):
                    if pos == 0:
                        ...
                        if be_armed:
                            _lock = entry + be_lock_frac * risk * (1 if pos > 0 else -1)
                            stop = max(stop, _lock) if pos > 0 else min(stop, _lock)
                        ...
                        if pos > 0:
                            if sl[k] <= stop:                       # stop first (pessimistic)
                                ...
                                pos = 0
                                if reenter_stop > 0 and _stops_taken < reenter_stop and raw <= -0.5 * risk:
                                    _stops_taken += 1
                                    continue
                                break
                            ...
                            if be_after_R > 0 and not be_armed and sc[k] >= be_lvl:
                                be_armed = True
                            if be_after_bars > 0 and not be_armed and (k - ek) >= be_after_bars:
                                be_armed = True
