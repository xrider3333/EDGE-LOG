...
- **The parent is DECLARED, not guessed:** every pinned variant carries `_AUGUR_PARENT` (e.g. `NOISE_1_1_SBS_V90.py` -> `NOISE_1_0.py`, `ORB_3_6_C2.py` -> `ORB_3_6.py`). Re-run THAT file, same window + master. A validate CANNOT take a custom grid - `run_validate()` has no grid parameter and builds its space from the strategy file DEFAULT_PARAMS - so the file itself must have open ranges. The BUILDER CUSTOM grid only reaches grid jobs.
