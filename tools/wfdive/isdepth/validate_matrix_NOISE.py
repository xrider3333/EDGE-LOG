...
PER-FAMILY FIX (NOISE, run #304, 2026-09-16 -- see AUTHOR_NOISE.md / task notes for why this
copy exists instead of editing the shared script):

    load_matrix() in the original script returns the raw `numpy.lib.npyio.NpzFile` from
    `np.load()` un-materialized. numpy's NpzFile does NOT cache a decompressed array across
    repeated `z["key"]` accesses -- each `z["entry_bar"]` (etc.) re-opens the underlying
    zip member and re-inflates the WHOLE array from scratch, every single call. `config_trades()`
    calls `z["offsets"]`/`z["entry_bar"]`/`z["exit_bar"]`/`z["net"]`/`z["entry_ns"]` once per
    config, and `check_fold()` calls `config_trades()` once per sampler candidate per fold
    (n_trials=250 x 8 folds = 2000 calls). On TTM's small matrix (1.67MB, ~350k pooled trades)
    this repeated inflate was cheap enough to be invisible (the scout's report: exact
    reproduction, no runtime complaint). NOISE's matrix is 45x bigger on disk (74.7MB, ~4.8M
    pooled trades across 1002 configs, this family trades ~300/yr vs TTM's ~22/yr) -- the SAME
    O(n_configs_checked) re-inflation pattern turned into a measured 391 CPU-seconds and still
    climbing after 6+ minutes wall time with no end in sight (observed 2026-09-16, killed before
    completion) -- effectively O(n_trials x n_folds) full-array decompressions of a ~150-300MB
    decompressed payload each, i.e. tens to hundreds of GB of redundant zlib inflation for one
    validate run. This is a PERFORMANCE bug in the shared script, not a correctness bug (the
    numbers it eventually would have produced are identical) -- surfaced by this family's trade
    density, not by TTM's.

    THE FIX (this file only): `load_matrix()` below materializes the five arrays this script
    actually reads (`offsets, entry_bar, exit_bar, net, entry_ns`) into a plain dict of real
    numpy arrays ONCE, right after `np.load()`, instead of returning the lazy NpzFile handle.
    Every other line of logic (`config_trades`, `window_stats`, `is_real`, `check_full_and_
    lockbox`, `check_fold`, `main`) is byte-identical to the shared script -- `d["key"]` on a
    plain dict indexes exactly like `z["key"]` on an NpzFile, so no other code needed to change.
    Also added `common.low_priority()` at the top of `main()` (the shared script never lowers
    its own priority since it assumes "no market data reload" implies "cheap" -- true for TTM,
    false for a family this trade-dense; the live fleet shares this box).
"""
import argparse
import json
import os
...
def load_matrix(run_id):
    ...
    _z = np.load(npz_path)
    # PER-FAMILY FIX (see module docstring): materialize once into plain ndarrays so
    # config_trades()'s repeated z["key"] indexing (up to n_trials * n_folds times) does
    # NOT re-inflate the whole compressed array from the zip on every call.
    z = {k: _z[k] for k in ("offsets", "entry_bar", "exit_bar", "net", "entry_ns")}
...
def main():
    ...
    common.low_priority()  # PER-FAMILY FIX: shared script never lowers priority (see docstring)
