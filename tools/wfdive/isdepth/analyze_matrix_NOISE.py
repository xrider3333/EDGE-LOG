...
PER-FAMILY FIX (NOISE, run #304, 2026-09-16): the shared script's `load_matrix()` returns the
raw `np.load()` NpzFile un-materialized, and `build_pool()` below indexes `z["entry_ns"]`,
`z["net"]`, `z["entry_bar"]` ONCE PER CONFIG inside its `for local_id, cfg_idx in
enumerate(cfg_order)` loop (~1002 iterations for this family's c_total). numpy's NpzFile does
NOT cache a decompressed array across repeated `z["key"]` calls -- each access re-inflates the
WHOLE compressed array from the zip member from scratch. On TTM's small matrix (1.67MB) this
was invisible; on NOISE's 74.7MB / ~4.8M-pooled-trade matrix the identical pattern that made
validate_matrix.py take 6+ minutes and climbing (see validate_matrix_NOISE.py's docstring for
the full measurement) would recur here at a similar order of magnitude, before any of A1-A4 even
starts. THE FIX (this file only): `load_matrix()` materializes the five arrays this script reads
into a plain dict of real ndarrays ONCE, so every `z["key"]` in `build_pool()` (the only place
this file touches the npz) indexes a plain dict/array instead of an NpzFile -- zero other logic
changed. (Unlike validate_matrix.py, this shared script already calls `common.low_priority()`
and wraps its work in `common.cpu_lock()` -- no change needed there.)
...
def load_matrix(run_id):
    ...
    # build_pool()'s per-config z["key"] indexing does NOT re-inflate the whole compressed
    # array from the zip on every one of the ~c_total loop iterations.
