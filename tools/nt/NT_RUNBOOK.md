...
- **EdgeLogENGUQ1m — RegimeLen: DEPLOYED 2026-09-08 21:34** (source copied from tools/nt, headless build 0 errors, `NinjaTrader.Custom.dll` swapped with NT stopped, previous dll kept as `.bak-20260908`; **the R2 crown knobs are SET on `EdgeLogENGUQ1m` since 21:41** - TlLen 206 / EmaLen 220 / BufAtr 0.3 / MinBrk 1.6 / AtrLen 52 / VolMult 1.1 / StopMult 1.0 / ActR 1.5 / TrailFrac 2.5 / BreakevenR 2.0 / LimitAtr 0.55 / RegimeLen 10 / ErLen 100 / ErTh 0.0, applied through the bridge (disable -> `/strategy/setparam` x14 -> `/strategy/check` ok -> enable; Realtime, no orphan order). The first `nt_recover.ps1` run was rejected at the Welcome window ("Incorrect username or password"), the second logged in with the same stored credential - a transient; retry ONCE, then stop and tell the owner. UNTESTED LIVE PATHS: the limit entry, the efficiency ratio and the regime gate had never run on the demo before this night - the nightly reconcile against paper leg `ENGUQ_335` is the parity check from 2026-09-09 on). History of the knob: the one
