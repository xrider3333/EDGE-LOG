...
| **H** | **Breakeven-after-R** (`be_after_R` param) — move stop→entry once unrealized ≥ X·risk | **HIGH** | ◐ PARAM SHIPPED, validate queued | **Fine sweep done (0.1 steps, 0–4R; §4.13):** NOT monotonic — **0.1–0.5R is a trap** (great full-history MAR, but lockbox PF collapses to 1.27–1.34: wicked out of good trades); **0.9–1.3R is a 5-wide robust plateau** (net $571–590k ≥ baseline, full DD ~−30%, lockbox PF 1.55–1.59, LB DD to −$11.5k, MAR 20–21.5 vs 14.1); ≥1.6R fades to baseline. Best cells: 1.0R (MAR 21.5) / 1.2R (net $590k). Param shipped as `ORB_3_0_BE.py` (137-lock, only be_after_R free; invariant $0-diff vs engine + sweep). Auto-Validate queued 2026-07-10 → full report card comparable to #137. |
