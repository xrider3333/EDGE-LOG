...
| **F** | **Consolidate — durable code + sizing/MAR in the app** | HIGH leverage | ☑ DONE | `augur_engine/sizing.py` (rp × time × side overlay); `tools/orb_edge_report.py` + `tools/orb_sized_backtest.py`; `run_grid(rank_by="mar")`; `history.py` `mar` field. **Live in the web app:** MAR column (v44.5) + a **SIZING OVERLAY recommendation card on ORB run reports (v44.6)**. **Backend fully wired:** `engine.run_backtest(sizing=…)` → `res["sizing"]` (headline unchanged), and the runner honors a `sizing` field on backtest jobs. *Remaining polish:* an interactive per-run "compute overlay" button (async job round-trip). |
