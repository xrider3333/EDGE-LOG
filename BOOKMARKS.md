...
- **Correlation measured (`tools/r37d_es_nq_corr.py`): ES leg vs NQ leg, exit-day PnL corr 0.20, monthly 0.21; 685 shared entry days of 1,148 (NQ) / 1,262 (ES). Pooled 1:1 (daily-aggregated): NQ alone n/DD 13.17, ES alone 10.76, NQ+ES $720,491 / DD $46,787 / **n/DD 15.40** — a RETURN diversifier, not just an account diversifier.** *(CORRECTED 2026-09-09: the first read of this line said ES alone 7.5 and pooled 16.1. Those came from a cumsum over an UNSORTED daily index — the same `pd.concat` trap a sibling documented in round 41 — so both drawdowns were wrong. The conclusion is unchanged: pooling still lifts net/DD 17% over NQ alone, and the corrected ES-alone 10.76 now agrees exactly with the round-37 single-leg read.)*
