        ...
        for _sn, _fn in _tilt_schemes:
            try:
                ...
                t = {"model": str(m), "scheme": _sn, "crownable": False,
                     ...
                     # v68.7 (owner: "how many small/medium/large trades did it do?")
                     "sizes": {"small": int((w < 0.75).sum()),
                               "normal": int(((w >= 0.75) & (w <= 1.5)).sum()),
                               "big": int((w > 1.5).sum()), "dropped": 0},
                     # v68.9 (owner: "we dont see what the bouncer level is"): the exact size
                     #   rule this column used, so the UI never leaves it to guesswork.
        ...
        for m, _pb in model_prob.items():
            ...
            try:
                ...
                hrow = {"model": str(m), "floor": _hth, "scheme": "linear",
                        ...
                        "max_size": round(float(w.max()), 2),
                        "sizes": {"small": int(((w > 0) & (w < 0.75)).sum()),
                                  "normal": int(((w >= 0.75) & (w <= 1.5)).sum()),
                                  "big": int((w > 1.5).sum()),
