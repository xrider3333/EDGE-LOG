...
| H | ROC % / YR + SORTINO rows and the MANAGER watch list (owner go via MANAGER 2026-09-26) | In progress. `tools/runboard_watch.py` ships the watch-list backing tool this round (one Firestore doc, add/verdict/note/remove/import commands, --dry preview), read by the WATCH chip's LANE VERDICT row on RUNBOARD. ROC %/YR + SORTINO matrix rows not started. |
